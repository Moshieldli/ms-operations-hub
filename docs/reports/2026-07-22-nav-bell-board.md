...
## Same-day follow-up (rev 63) — register sound redone

The /finance chime is now `public/sounds/register.wav` — a big mechanical CHA-CHING (double bell +
lever clunk + drawer slide + coin rattle + stop thock, 1.7s, normalized 0.95 vs the old in-browser
synth's ~0.2 gain) generated by `scripts/make-register-sound.ts`; TV sale-bell sounds untouched;
verified on a simulated clearance (8/8 — play attempted + file served + visuals). **During
verification the feature caught its first REAL payment: Rob Kurtz's $479.31 cleared and rang in
alongside the seeded test row** (paused roster 10 → 9 accounts).
