...
- **Contract type** (`summary.contractTypeBreakdown`) = those same Active Services grouped by `NormalizedContract.contractType` (from `contract.agreement.name` — the granular Pocomos "Contract Type" pick-list, e.g. `Mosquito Control`, `Add On Tick Control`, `Ant Control`, `Natural Mosquito Control`), sorted descending by count. Blank contract types fall into a `"(no contract type)"` bucket. Rendered as a "Contract type" card on `/sales` and a compact grid on `/tv/sales`. **Note:** this is deliberately NOT `pest_contract.service_type.name` — that field is the broad 11-category Service Type with a literal `Other` catch-all and is too coarse for the breakdown. `NormalizedContract.serviceType` still carries the broad value for any other use.
