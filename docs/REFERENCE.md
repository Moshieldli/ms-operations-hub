...
- **Email detection is best-effort** (`extractEmailSent`): scans this-call `call_notes[]` + `events.last_event` for an "email … sent" marker and extracts the trailing template/subject name. No email-disposition payload has been captured yet, so the pattern is unproven against a real send — when in doubt it returns null and the line is skipped.
