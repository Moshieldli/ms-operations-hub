...
**Scheduled-today section (added 2026-07-07, rev 13).** An overdue row whose **next scheduled service is today** (`next_service_date == today`, **Eastern**) is being handled today, so it is pulled into its **own green "Scheduled today" card** rendered between the stat boxes and the overdue table, and **excluded from the overdue COUNT** (the Overdue stat keeps an "Excludes N scheduled for today" sub-line pointing at it). It is NO LONGER in the overdue table. Computed at **read time** in `getOverdueReport()` (`refresh.ts`, `easternTodayIso()` splits `allOverdue` into `overdue[]` + `scheduledToday[]` via a per-row `scheduled_today` flag; `counts.scheduledToday`), NOT stored — so "today" is always the day you're viewing, independent of when the 06:00 refresh ran.
...
**Ops definition (2026-07-07, rev 14 — supersedes the tag-based rev 12/13 model):** the question is simply *"of customers who received ≥1 completed mosquito service (not Event Spray) in year Y, how many are receiving a mosquito service in year Y+1?"* Prefer completed-service evidence over tags wherever we have it; tags are a fallback proxy where service-in-year can't be determined in bulk.

**`servedInYear(rec, Y)`** (`computeReturnRates()` in `sales-taxonomy.ts`) decides real-customer membership:
- **Identity + season:** a **mosquito-family contract** (`service_type` ∈ the §5.5 mosquito set, Event Spray excluded — only 18 event-spray-only customers exist) whose OWN per-contract tags carry a `"{Y} -"` tag. Tags are the only bulk per-season signal, so they identify the mosquito contract and which seasons it was enrolled for.
- **Completed-service gate:**
  - **CURRENT (in-progress) season** → the customer must still be **LIVE** (Active, or On-Hold). An Inactive/cancelled customer is NOT receiving service even with an auto-renew tag. **This is the bug fix** — a `"{CY} -"` tag on a cancelled customer no longer counts as a return.
  - **COMPLETED past season** → LIVE now (continuity proxy) **OR** direct evidence that `last_service_date`'s year ≥ Y. An Inactive customer whose last service **pre-dates Y** auto-renewed then cancelled *before* the season materialised (never got a Y service) → excluded.
- **On-Hold** counts as served/returned (`ON_HOLD_COUNTS = true`; paused ≠ cancelled). Sensitivity is ~0.1–0.3pp (see below).

**Bulk-evidence & confidence.** `last_service_date` (from `/customers/data` / the enriched `customers` table) is the **most-recent** service only and is **any-type, not mosquito-specific**. It gives a definitive YES for last-service==Y and a definitive NO for last-service<Y, but **cannot confirm a specific past year's service for a still-active customer whose last service is later** — those rely on the season tag as a continuity proxy. A literal per-year completed-mosquito-service count would need a **full service-history scrape of the cancelled cohort** (the refresh only scrapes history for the ~21% currently-eligible add-on customers, not the historical 2024/2025 cohort). Consequence:
- **25→26 is HIGH confidence** — its numerator is anchored on **live status** (2026 = "is receiving service now"), directly observable. Of 956 returners, 953 are backed by live-active/last-service≥2026; only 3 rest on the tag proxy.
- **24→25 is a tag-anchored ESTIMATE** — both the "served 2024" and "served 2025" sides fall back to the season tag + active-continuity for the large active cohort (whose last service is now 2026 and so can't confirm a specific earlier year). Direct service-date evidence only sharpens the edges (definitively drops customers whose last service pre-dates the year). Treat 24→25 as ±a few points until a full history scrape lands. The card marks it *(est.)*.

**Live numbers (verified via `getSalesTaxonomy()`, 2026-07-07, rev 14):** 24→25 = **80.5%** (1,037 / 1,288), 25→26 = **72.6%** (956 / 1,317); 18 event-spray-only excluded; on-hold in the numerators = 4 (24→25) and 5 (25→26). **On-hold sensitivity:** excluding On-Hold gives 80.4% (1,036/1,288) and 72.3% (951/1,316) — negligible. **Old (rev 13) vs new:** 24→25 73.4%→**80.5%**, 25→26 74.0%→**72.6%**. The 25→26 rate *dropped* because the bug fix removed 123 cancelled-but-auto-renew-tagged customers from the numerator; the 24→25 rate *rose* because the new definition correctly credits end-of-2025 churners who genuinely received a 2025 service (the old model wrongly dropped them as "mid-season cancels").

**Returning-bucket reconciliation (task #2, 2026-07-07).** `scripts/diag-return-fix.ts` writes `audit-returning-not-counted.csv` (gitignored): **55** customers are in the `/sales` "Returning" (RETAINED, active-only, union-tag) bucket but NOT in the 25→26 numerator. All 55 are **correctly excluded** (no bug): **51** have no 2025 mosquito tag (they're brand-new or returning-lapsed *in* 2026, so they were never a real 2025 customer — nothing to return *from*), **3** have no mosquito contract at all (their 2026 continuation tag is on a non-mosquito service), and **1** carries its 2026 continuation tag on a non-mosquito contract. The numerator (mosquito-specific, requires a real *prior* season) is a subset of the broader RETAINED bucket by design. The earlier rev-13 `audit-return-gap.csv` (130 numerator-not-in-bucket rows) is now historical — those 128 inactive/on-hold rows are exactly what the bug fix removed.
...
8. **`/sales` return rate — RESOLVED (2026-07-07, rev 14).** Ops gave a single definition (§5.8): "received ≥1 completed mosquito service in Y, and receiving service in Y+1?" — so the old dual primary/excl-mid-season denominator is gone (one rate per pair). The rev-12/13 numerator bug (counting cancelled auto-renew-tagged customers as returns) is fixed via `servedInYear()`. **Still open (accuracy, not correctness):** the past-year sides (2024/2025 "served") fall back to the season **tag as a continuity proxy** for still-active customers because `last_service_date` is most-recent-only + any-type; a literal per-year mosquito-service count for the cancelled cohort needs a full **service-history scrape** (mirror the inactive-enrichment cron; ties to open #6's deactivation-date scrape). Until then, 24→25 is a tag-anchored estimate (marked *est.* on the card); 25→26 is live-status-anchored and exact.
