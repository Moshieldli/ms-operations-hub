...
since rev 28. Folder `1UsODcBn0JsGMEzsZQQ1CmCVMgxe1Z0nM` ("Payroll Calculator - MS → 2026"), one
sheet per week ("MS Payroll Calculator - YYYY-MM-DD", Friday week-ending), one tab per tech.
...
`node:crypto` — **no `googleapis` dependency** for three REST calls. Nightly cron
`/api/cron/referrals` (`0 10 * * *`) scans the newest 6 sheets. ⚠️ **DORMANT until a service account
is provisioned**: `hasDriveCredentials()` is false without `GOOGLE_SERVICE_ACCOUNT_EMAIL` /
`GOOGLE_PRIVATE_KEY`, so the cron **no-ops with a reason and returns 200** rather than failing.
Until then `referral_awards` is the **manual/seed path** (`scripts/seed-referrals.ts`), and the
payroll sheets stay source of truth either way — the cron takes over with zero code change once the
env vars exist. ⚠️ `recordAwards` (repeat-avoidance) records only the six weekly SEATS, never the
trophies — two trophies share `award_id="referral"` and would collide on `(week_start, award_id)`.
