...
name big, stat line) · YTD ticker. **No bottom table on this variant.** Single column of stacked
tiles at Yodeck widths; **two columns only at ≥900px** (`TWO_COL_MIN_WIDTH`).

**ADAPTIVE AWARDS (rev 30).** The real Yodeck slot is SHORT (~470×430): after the weather strip and
ticker, six tiles get ~44px each — present but unreadable across a room. So the awards region
measures itself and picks a mode: **fits (≥76px per tile row) → all six, static; doesn't fit →
rotate 3 at a time, cross-fading every 15s, forever** (no interaction). 550×700, 600×900 and the
wide 1200×600 (two columns) all show six at once; only the short slot rotates. Measuring is
feedback-loop-safe: the awards region is the only `flex-1` element, so its height comes from the
`shrink-0` chrome around it, never from its own content.

**Tile type is sized in `em` off the MEASURED row height, not viewport units** — the parent computes
`basePx ≈ rowHeight × 0.20` (clamped 9–26px) and the tile's internals are all `em`. Viewport-relative
type can't know how tall its row is: at 550×700 it overflowed the tile and clipped the name and stat
mid-glyph while the DOCUMENT fit perfectly, so the document-level overflow check sailed straight past
it. `scripts/verify-tv-techs-tall.ts` now asserts **per-tile** `scrollHeight ≤ clientHeight` via a
`data-award-tile` hook, at every size — that's the check that catches this class of bug.
