...
**Layout, top to bottom:** weather strip · "Tech Board" + week · the six awards as one-line rows
(emoji + FIRST name + stat) · YTD ticker. **No bottom table on this variant.**

**Responsive by design — no breakpoints.** The Yodeck webpage widget IS the viewport, so every size
is `clamp(min, Nvw, max)` rather than a fixed px or a Tailwind step; Yodeck's zoom just changes the
viewport and the type follows. Verified at 500×450, 550×700 and 600×900 with zero overflow (a TV
widget cannot scroll). **Priority when short:** weather, header and ticker are `shrink-0` so they
always render; the awards block is the single `flex-1 min-h-0` region, so a short container
compresses the awards instead of pushing anything off-screen.
