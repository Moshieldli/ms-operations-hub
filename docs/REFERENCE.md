...
**Nav dropdowns.** `components/nav.tsx` — the `NavDropdown` (click-driven, closes on outside-click/Escape/navigation, works on touch; a tab highlights on its own page AND any child page via `isActive` = `pathname === href || startsWith(href + "/")`). Three dropdowns now: **Leads** (Close rate / Follow-ups), **Service** (Overdue sprays → `/service/overdue`, Respray performance → `/service/resprays`), **Sales** (Sales → `/sales`, Paused — open balance → `/finance`). Plus a standalone **Finance** tab. **Choice made:** the Sales "Paused" item links to the new `/finance` page (the alternative was a `/service/overdue` anchor); `/finance` is the cleaner home and shares the exact paused component. Note `/finance` appears both as its own tab and as the Sales "Paused" shortcut — intentional convenience; only the Finance tab highlights on `/finance` (the Sales parent keys on `/sales`). The `/service` registry landing page still exists by URL but is no longer the nav entry point (the dropdown links the two live reports directly).
