...
**Ops-canonical definition (2026-07-13, rev 16 — ≥1 completed service with a late-one-off carve-out; supersedes the rev-15 ≥2 threshold):** *"of the real customers of year Y, how many were real customers of year Y+1?"* A **real customer of year Y** = received **≥1** completed mosquito-family service in Y (Event Spray EXCLUDED), **EXCEPT** a customer whose **only** Y spray landed **after `LATE_SEASON_CUTOFF` (Aug 15)** — that single late spray is an extended-season one-off sale, not a real customer. A customer with 2+ sprays is real regardless of when they fell (not a one-off). The **same** test defines the denominator (real year-Y customer) and the numerator (returned = real customer of Y+1). `LATE_SEASON_CUTOFF` (`{ month: 8, day: 15 }`, year-relative) + `RETURN_RATE_MIN_COVERAGE_PCT` are documented constants in `sales-taxonomy.ts`. The carve-out needs per-spray dates: `mosquito_service_counts` now carries `first_service_date` / `last_service_date` (earliest/last completed mosquito spray per year); the single-spray customer's one spray IS the earliest, so `isLateSeasonSpray(first)` decides it.

**Why the change:** the ≥2 threshold dropped genuine single-visit early-season customers from the denominator; the new rule counts anyone who got a real (non-late) spray, while still excluding pure extended-season one-offs. The card also surfaces how many single-late-spray customers the carve-out excludes per year (`ReturnRatePair.excludedLateFrom` / `excludedLateTo`).
...
8. **`/sales` return rate — definition RESOLVED (rev 15); 24→25 BLOCKED on full history.** Ops-canonical metric is now completed-service-count based: ≥2 completed mosquito services in Y and in Y+1 (§5.8). Built as a nightly resumable service-count scrape (`serviceCounts.ts` → `mosquito_service_counts`, 100% covered). **25→26 = 75.9% (911/1,200), live and exact.** **OPEN:** the Pocomos service-history table is **truncated to the most recent ~30 services (~1 season)**, so 2024 is gone for continuing customers and **24→25 is not computable** (shows *n/a — needs full service history* on the card). To unblock 24→25 we need a full-history source: parse the **PDF export** (`/customer/{id}/contract/{cid}/history/download` returns application/pdf), or find a paginated/date-ranged services endpoint. Probe: `scripts/probe-history-window.ts`.
