...
**Last updated:** July 22, 2026 (rev 63 — **/finance register sound REDONE as the big mechanical CHA-CHING** (§5.14b): now a committed file `public/sounds/register.wav` (1.7s — double bell strike C7 + lever clunk + drawer-slide noise + coin rattle + stop thock; generator `scripts/make-register-sound.ts`, deterministic LCG noise, normalized 0.95 vs the old in-browser chime's ~0.2 gain — much louder, tanh-limited, no clipping; swap the file to change the sound). The component plays the file (HTMLAudio) instead of a WebAudio synth; session-start unlock + replay affordance unchanged; /tv/sales bell sounds untouched. **First REAL clearance caught mid-verify: Rob Kurtz $479.31** (roster 10→9, $2,888.84→$2,409.53). rev 62 — **BOARD V2: true mirror of the physical weekly board** (§5.19/§5.20): the grid is now the **CURRENT Sun–Fri SERVICE week (6 columns, always)** — not a rolling today-forward window — flipping on Saturday (`resprays.ts::weekStart` + Saturday bump; distinct from the Sat–Fri SALES week); Sunday renders even when empty, today is ringed with a TODAY badge, header shows the week range. **Split enforced:** `/tv/board` has ZERO editing UI (verified — 0 form elements); `/service/board` is the admin mirror and gains a **`?week=YYYY-MM-DD` review override** ("(review)" tag; the TV never passes it). **Fallback days keep the SAME row format** with tech "—" (labeled "not on sheet yet · Pocomos (mosquito only)" — probed: those counts are real per-route next-service tallies, e.g. 402=15, but UNDERCOUNT the sheet's ~30/route because the table only holds eligible mosquito customers' next service; capped at 9 rows + "+N more"). **Markers:** ant bug marker verified on REAL sheet days (Cesar "ANT" 6/15, Nathaniel "608, ANT" 6/16 — rendered via the review override; "Ant needs 3 dry days" caution is forecast-driven); weekly/special codes (WF/WG/RLW/TKO/ASAP/TMP…) render as violet pills vs mono-sky numerics, with a legend chip; SERVICE CODES legend titled. **Fidelity:** names/towns never truncate (break-words), OFF/OUT render as status rows, day Notes line renders, sheet typos mirror verbatim (e.g. Jason 7/22 "207" stops). **⚠️ Parser landmine fixed: the sheet's label row sometimes says "Day Code" (with a space) — the strict `/^daycode$/` match fell back to the Van column and SWAPPED daycode/van on those days** (seen on Jun 17–19); now `/^day\s*code$/i`. **Announcements:** seeded from the physical board (THIS WEEK all-weekly-synthetic→NATURAL / NEXT WEEK →SYNTHETIC; seed script only fills empty fields) + new **URGENT ANNOUNCEMENT** (`board_announcements.urgent`, ≤300 chars) — big red banner on BOTH boards, edited only on `/service/board`. Verified live at 1080p (16/16: SVG-only, no clipping, zero edit UI) — `scripts/verify-board-v2.ts`. rev 61 — **`/finance` collections CTA redesigned** (§5.14b): "Start collections session" is now THE primary action — large emerald-gradient button (lucide DollarSign badge; sub-line "Track payments as they land" — deliberately audio-neutral so the register sound stays a surprise) with a soft glow pulse (2.8s box-shadow keyframes, disabled under prefers-reduced-motion); while LIVE it transforms into an equally obvious active panel (2px emerald border, pulsing dot, "Collections session LIVE", running tally, separate Stop button) — not a toggled label. rev 60 — **NEW-SALE BELL on `/tv/sales`** (§5.22): the shop TV (a real Chrome kiosk, NOT Yodeck — sound viable) rings when the live `buckets.NEW` count CLIMBS: sale.wav + a "+N NEW SALE" splash + New-tile flash; drops (tag corrections) stay silent. **Weekly tally "This week: X / 25"** = live NEW − `snapshots.new_count` at the week-start SATURDAY (Sat–Fri SALES week via `startOfSaturdayWeek` — distinct from the Sun–Fri service week), rendered on the TV header + subtle on `/sales` (browser page = tally only, NO auto-sound). **Milestones** at 10 (milestone-10.wav) and 25 = goal (milestone-25.wav) fire ONCE per week — persisted server-side in `sync_state` (`sale_milestones_{weekStart}`) via `GET/POST /api/sales/week-tally`, so kiosk reloads can't re-ring; a climb that crosses a milestone plays the milestone sound only. Three ORIGINAL synthesized WAVs committed to `/public/sounds/` (generator `scripts/make-sounds.ts` — swap a file to change a sound). Last-seen count in localStorage: reloads don't re-ring old climbs, but sales landed while the tab was closed ring once. Autoplay-blocked → visual runs + one-time "Enable sound" affordance (or set Chrome site permission Sound=Allow on the kiosk); mute persisted. Bell series = categorize `buckets.NEW` (the snapshot series), deliberately NOT the taxonomy New tile (different definition — can't be diffed against snapshots). `/sales` page h1 renamed "Customers" to match the rev-59 nav. rev 59 — **NAV TAXONOMY** (§5.14): top bar is now **Customers · Leads · Service · Finance · Texting · Requests · TV**. "Sales" tab RENAMED **"Customers"** (label only — routes `/sales` + `/tv/sales` unchanged) and demoted from dropdown to a **plain tab** (the "Paused — open balance" child is gone; Finance owns `/finance`, so no double-highlight is even possible now). **`/combined` and `/calling` placeholder pages DELETED** with permanent redirects `/combined`→`/sales`, `/calling`→`/leads` (`next.config.mjs redirects()`; the real calling dashboard arrives with the PB backfill and reclaims its nav spot then). TV dropdown gains **Route board (`/tv/board`)** — the sweep's one URL-only page — all TV children new-tab. Home page cards realigned to the taxonomy. **STANDING RULE added to CLAUDE.md + the dashboard-conventions skill: every new page ships WITH its nav entry in the same build — not in the nav = not shipped.** Verified live by `scripts/verify-nav-taxonomy.ts` (labels/order/hrefs, dropdown children, new-tab+noopener, redirects, single-active-tab on /finance). rev 58 — **Texting password rotated** (§5.7): `TEXTING_PASSWORD` (Vercel Production) changed + redeployed (edge middleware only picks env up on deploy); all existing `texting_auth` cookies invalidated by construction (the cookie is a SHA-256 of the password), so each person enters the new password once. Also fixed the parent-company typo doc-wide: **Proganic** (no second R), was "Progranic". rev 57 — **PROBE, docs-only: `open_balance` = OVERDUE balance, installment-safe** (§5.14b). Ops flagged that paused-balance customers are those with OVERDUE payments (installment plans may still owe future installments after a collection). Probed the Unpaid-Invoices pull per-invoice: all 83 rows have due dates ≤ today (range 2025-04-01 → **exactly today**; statuses only "Past due"/"Due"; a live "Installment 1 of 4" row proves installment invoices surface only once due) — **the report returns NO future-due invoices** even though our request window runs through next year (the form's aging buckets cover past-due ages only). So the >0 → $0 clearance zero-test is already correct for installment customers, and future installments never inflate `open_balance`. No code change; probes `scripts/probe-unpaid-duedates{,2}.ts`. rev 56 — **Feedback bubble: "Record screen" with mic narration** (§5.18): third capture option next to Attach file / Take screenshot (both untouched — html2canvas + markup stay as-is). `getDisplayMedia` screen + `getUserMedia` mic (echoCancellation+noiseSuppression) merged into one `MediaRecorder`; **mic denied → silent recording with a visible note, never aborts**; feature-detected (no getDisplayMedia/MediaRecorder → button never renders, so no mobile-Safari dead end). Max 60s with countdown + Stop + red mic-hot indicator; panel hidden while recording; user ending the share from browser chrome stops cleanly; ALL tracks stopped on stop/discard/cancel/unmount (no orphaned mic). Preview modal WITH audio → Attach / Discard & re-record. **Size:** `MAX_VIDEO_BYTES = 3 MB` (the ~4.5 MB Vercel body limit is the ceiling, not the 60s; base64 ×1.37): vp9/opus at 350kbps video + 32kbps audio probed at **~1.24 MB raw per 60s** (fake source; worst case ≈2.9 MB), auto-stop at 95% of cap with a "size limit reached" note. **⚠️ Landmine found live: `recorder.mimeType` carries `;codecs=vp9,opus` — the COMMA breaks data-URI parsing**; the client strips parameters (bare `video/webm`), the server regexes tolerate comma-free params. New nullable `feedback.video_data_uri` (validated `data:video/...;base64,` or dropped; a submission may carry image, video, both, or neither), list gains `hasVideo` (never the blob), new `GET /api/feedback/{id}/video` serves the real binary with the stored content-type; `/requests` rows get a Video tile → full-screen `<video controls autoPlay>` (click = gesture, audio plays). Rate limit unchanged. E2E-verified live with a real Chromium MediaRecorder (fake source/mic): record→attach→submit→store→serve (`video/webm`, 139 KB/6s)→/requests playback; test row deleted. Also: /finance "Updated Updated" double-word fixed. rev 55 — **CASH-REGISTER moment on `/finance` + Collections Mode** (§5.14b, display-only — the hub never touches payments): new `balance_clearances` table logs a paused customer's open balance going **>0 → $0** (FULL clears only; partial payments update the stored balance and never celebrate), ring-once via a per-(customer, UTC-day) unique index + `ON CONFLICT DO NOTHING`. Detection in BOTH the mosquito refresh (prior-balance snapshot read before the upsert, diffed against the fresh unpaid pull; still-ELIGIBLE customers only, so a cancelled row vanishing never rings; empty-report guard) AND the new `POST /api/finance/collections-check` (fresh Unpaid-Invoices pull — **measured 2.4–3.4s**, two back-to-back pulls agreed exactly — diffed against the paused roster, 10s soft-lock coalesces concurrent tabs). `/finance` UI: passive "Collected $X since you last looked (N customers)" celebration off a localStorage marker (first visit initializes silently), WebAudio-synthesized cha-ching (no sound asset; autoplay-blocked → "Replay sound" affordance; mute toggle persisted) + green flying dollar amounts (prefers-reduced-motion respected), and **Collections Mode**: "Start collections session" re-checks on `visibilitychange` (tabbing back from Pocomos) + a 20s fallback poll, rings per-clear (the start click unlocks the AudioContext so autoplay never blocks mid-session), flashes the row green and slides it out of the roster, keeps a session tally, auto-stops after 10 quiet minutes; per-tab persistence via sessionStorage. Cleared rows get `status='cleared_balance'` (rendered nowhere) until the next full refresh recomputes them. `RefreshMeta.balanceClearances` added. Never on `/tv/*`. rev 54 — **Historical webhook-notes backfill EXECUTED + two resolve landmines** (§5.2/§5.4): **251 of 290** dead-parser-era calls (Jun 4 – Jul 16; Win-back 243 · Lead 37; **202 with an `Email sent:` line**) now have v2 Pocomos notes carrying the original call date (`Win-back call — Left Message · 6/4/26`); test-folder dials excluded (ops), safety-checked (0 today/wellness rows; first dry run correctly ABORTED on a classifier using TODAY's folder state for historical calls — fixed to call-time payload data). **Landmine 1: `find-customer-by-office` is a WEB-SESSION endpoint** — the JWT client gets a 200 login-redirect JSON, so the route's resolve had silently never worked; **and it returns EMPTY for cancelled customers** regardless of params, so win-back ids need the **contact-details bridge** (email → phone+last-name vs the full JWT customer list; 242 of 250 untracked rows bridged) — the live route now falls back to the enriched `customers` table by phone+last-name. **Landmine 2: the lead-note endpoint (`/jwt/{office}/lead/{id}/note`) is DEAD** — 404 on all 9 real leads; 39 lead-record rows remain `note_written=FALSE` (retryable) until a working lead-note path is found (§9). rev 53 — **"Email sent:" line LIVE + replay gate + test-note cleanup** (§5.4): one-touch emails are recorded as a same-payload contact-notes entry (`-- {ts} by {agent} -- Email sent: {subject}`) — `extractEmailSent` now reads that cluster with a 3-min timestamp guard (a re-dial payload carrying only the PREVIOUS call's email entry is correctly rejected; 4/4 real payloads pass). Fixed the `ENTRY_HEADER` regex for the modern `by {agent}` header segment (entry parsing was silently dead). New **call_id replay gate**: a `webhook_log` row with the same `call_id` + `note_written=TRUE` hard-skips the note write (live-proven; also the backfill dedupe). Deleted the three old-format TEST notes (ops-authorized; delete = `POST /customer/{id}/note/{noteId}/delete`, a `data-method=post` action — 405 on GET as expected). rev 52 — **Pocomos note format redesigned + two loop-guard/recording bugs** (§5.4): notes are now `{Campaign} call — {disposition}` (campaign from the dialed folder: Wellness/Lead/Win-back/PhoneBurner) + optional `Email sent: {name}` + `{CSR} · {duration}s` + optional `Notes:` — **no recording line** (every payload recording URL 404s even intact — S3 object missing; PB fills the field on unconnected VM drops) and no `Notes: (none)`. **Loop-guard landmine fixed:** Pocomos stores 📞 as `?`, so the old `startsWith("📞 PhoneBurner Call —")` read-back dedup NEVER matched — replaced by the shared emoji-free `PB_NOTE_GUARD` regex (new format + all stored legacy forms), used by the writer AND `notes.ts::classifySource` (leadSync/notesRefresh). Historical payloads replay into the new format (backfill-ready). Also same-day: first-dial webhook failure root-caused — PB's real payload carries custom fields + `folder` at the TOP level, not under `contact` (§4 landmine; fix + DB bridge deployed; wellness fall-out verified end-to-end on 3 real calls). rev 51 — **Wellness-calls campaign** (§5.21): self-refilling PhoneBurner queue of active customers with **2+ completed mosquito sprays this season**; one dial attempt of ANY disposition removes them for the season. Two new EXEMPT folders (Queue `66255089` / Called `66255090` — NEVER police them), `mosquito_service_status.sprays_this_season` (aggregated from `respray_jobs` — **no per-customer scrape**; the spec assumed none existed, but the completed-jobs cache IS a bulk per-season count source, contract-independent; 5/5 spot-checks matched the service-history page), `wellness_calls` season-guard table (insert-FIRST idempotency), webhook fall-out (record → move to Called → DIRECT Pocomos note, no resolve step — the stored Customer ID is the internal url_id), daily 07:00 feeder cron (**shipped dry-run-gated**: cron path carries `?dryRun=1` until the ops go). `respray_jobs` gains the report's street `address`; notes-refresh Phase B now also covers the wellness Queue. Feeder dry run: roster 1,165 · 2+ sprays 1,137 · would-push **1,114** (9 paused, 2 no-phone, 12 shared-phone dupes). rev 50 — **/tv/board v2 (tech-first) + /service/board + compliments** (§5.19/§5.20). Master Routing CALENDAR is now PRIMARY: per-day tech-first rows (NAME/DayCode/Van/Towns/Stops), Pocomos fallback for unfilled days; ant + electric-blower markers, LC/LI/BK/GN/QU/WC legend, announcements panel (Neon, editable), new-customer box, day notes. New **/service/board** browser mirror (nav under Service) with announcement editor + shout-out form + management. **Compliments/shout-outs**: roster from column A ONLY of the Technician sheet (names only; 8 techs incl. Cesar/Emanuel), nightly cron; shout-out form (≤160 chars, localStorage name), 7-day panel on both boards (rotates on overflow, SVG megaphone), soft-delete. Yodeck-safe (SVG, no emoji, no clip, verified). rev 49 — **CALENDAR overlay live + feedback bubble upgrades** (§5.15/§5.18). Master Routing sheet now shared, so the corrected CALENDAR parser (label-anchored columns, placeholder Tech1..6 dropped, name variants kept) feeds tech-first rows into /tv/board — verified live (Nathaniel/Nick/Joseph/Daniel/Jason). Feedback bubble: **Take screenshot** (html2canvas, no permission prompt) → **markup step** (arrows/boxes/freehand, red default); **name now REQUIRED + localStorage-remembered**; /requests gains a **by-submitter filter + per-person counts**. rev 48 — **Referral scanner LIVE** (§5.15). Drive/Sheets APIs enabled + service-account creds set, so the nightly payroll scan runs for real. Fixed matchTechnician for the actual tab format **"LAST, F"** (first initial only) — surname-anchored + Levenshtein ≤2 + first-initial tie-break instead of two full tokens. Live scan matches the seed: Nicholas ← Channa Noiman, Nathaniel ← Mina Becher. ⚠️ The Master Routing sheet is still **403 (not shared)** with the service account, so /tv/board CALENDAR overlay stays dormant — see BACKLOG. rev 47 — **Two probes (no code): PB call-history + respray-weather.** **(TASK 5)** PhoneBurner DOES expose historical calls — structural probe of phoneburner.com/rest/1 (401=exists/404=no, calibrated) confirms **GET /dialsession (list) + /dialsession/{id} exist**; no /calls, /callanalytics, /reports, or contact-level call endpoints. Backfill path = page /dialsession (each session carries calls + dispositions + the dialing member/CSR). Webhook data can't attribute per-CSR (all 293 rows = the API-account user), so /dialsession is also how we would attribute Rena. Live volume not measured (PHONEBURNER_TOKEN sensitive in Vercel). **(TASK 6)** Rainfall is NOT a monthly driver: same-month Pearson r rain-vs-respray −0.19, rain-vs-cadence −0.06; weak lagged rain→next-month-respray +0.40 (n=9). YoY cadence decline is operational, not weather. Both findings in BACKLOG. rev 46 — **Emoji sweep on browser pages** — the last 🎉/🏆 on /sales, /service/overdue and /service/resprays are now lucide SVG (AllClearIcon = PartyPopper emerald, TrophyIcon = Trophy amber; components/ui/status-icons.tsx), consistent with the emoji-free TV boards. Polish only. rev 45 — **BUILD-SCHEDULE v1: /tv/board digital route schedule** (§5.19). Yodeck-safe TV board (SVG, no emoji, display-only): today + next 4 workdays, each with weather + the day's route codes/towns/stop counts. Schedule from Pocomos (mosquito_service_status next_service_date + route_code); daycode→towns from a snapshot of the Master Routing DAYCODES tab (routeTowns.ts); Electric-Blower marker from the live NT-Electric-Blower tag (fast customers-table query). Sheet-driven TECH names + ANT-day rain markers (masterRouting.ts) are DORMANT until the Drive/Sheets APIs are enabled — board runs fully off Pocomos meanwhile with a sheet-not-connected badge. rev 44 — **WC SEASON-BUCKET FIX + referral creds set** (§5.8). **(1) Westchester is YEAR ONE** (ops): WC territory was bought from a prior operator, so a WC customer's pre-CY tags aren't a season with us. A customer with the `0-Westchester` tag (or ZIP 105xx-108xx fallback) whose only prior-year evidence is a TAG (no spray history in our data) now counts **New**, not Season-Skipped. **Live: New 149 → 199, Season-Skipped 89 → 39 (50 WC moved), Returning + total unchanged.** New `isWestchester()` helper + a `zip` field on `Person`; /sales tile definitions updated. **(2) Referral service-account creds** (`payroll-reader@referral-hub-500201`) set + verified in `.env.local` and Vercel Production (key reconstructs to exact PEM; JWT authenticates) — but the live scan is **blocked on enabling the Google Drive + Sheets APIs** in GCP project referral-hub-500201 (console action). See BACKLOG. rev 43 — **Referral payroll scan is now YEAR-PROOF** (§5.15). Ops shared the **parent** "Payroll Calculator - MS" folder (not just 2026), so `resolveYearFolderId()` now finds the `${CURRENT_YEAR}` subfolder **by name** inside the parent (`1EP1fMZrPMaCnx3lY2rt3DYOM-v8kwAwF`) at runtime instead of hardcoding the 2026 folder ID — the `name='2026'` match skips the `0-Templates` sibling, and it's year-relative, so **January 2027 needs zero code/config change**. New env `PAYROLL_DRIVE_PARENT_FOLDER_ID` (defaulted); optional `PAYROLL_DRIVE_FOLDER_ID` still pins a folder for backfills. Missing year folder → clear error, cron skips (never crashes). **Yearly ritual: create a `{YEAR}` folder in the parent, nothing else.** Scanner still dormant until the service account is provisioned. rev 42 — **FEEDBACK-SYSTEM: in-dashboard feedback bubble + `/requests` review queue + prompt builder** (§5.18). Floating "Feedback" button bottom-right on every dashboard page (NOT on `/tv/*` kiosks); compact form takes text + optional name + optional image, auto-captures the page URL + timestamp, **downscales the image client-side** (fit 1600px JPEG) before upload. New `feedback` table stores images **base64 inline** (cheapest at our volume; ~2 MB cap, list returns a `hasImage` flag and serves the binary on demand). New top-nav **Requests** tab → `/requests`: newest-first list with text/page/submitter/date/thumbnail, click-to-cycle status (New → Selected → Shipped → Declined), status filters (Shipped stays visible). **Prompt builder**: tick items → Build prompt emits a paste-ready `/ship` prompt (numbered items + this repo's ritual header/footer) + copy button; included items auto-become Selected. No Pocomos, no cron, no auth (internal; POST lightly rate-limited). Verified end-to-end live. rev 41 — **TV-TECHS-REFERRAL: spinning referral trophy + month-long boost** (§5.15), the deferred rev-28 award, now unblocked. Data source is the **weekly PAYROLL Google Sheets** (folder 1UsODcBn0JsGMEzsZQQ1CmCVMgxe1Z0nM), NOT Pocomos: a referral = an OTHER PAY row of **exactly $50** with a customer name in NOTES (both halves load-bearing — a $10 "Westchester walkthrough" row proves the amount test). Scanned 6 weeks: **Nicholas → Channa Noiman, Nathaniel → Mina Becher**. Spinning gold trophy hero on both boards (`animate-spin-slow`, the reserved hook) naming the CUSTOMER, **no dollar amounts anywhere**; multiple referrers → multiple trophies. Month-long "boosted" gold-star badge on every tile a referrer wins (`getBoostedTechs`, week-ending + 1 month). New `referral_awards` table keyed (tech, customer) — the sheet repeats a referral across weeks. Ingestion via Drive+Sheets **read-only** APIs (service-account JWT signed with `node:crypto`, no `googleapis` dep); nightly cron `/api/cron/referrals`. ⚠️ **DORMANT until a Google service account is provisioned** — cron no-ops cleanly; `referral_awards` seeded by `scripts/seed-referrals.ts` meanwhile (payroll = source of truth either way). SVG only, verified no-clip/spin/boost at all sizes. rev 40 — **TV ticker: "Longest streak" → "Team best"** (§5.12). The ticker and the Clean Streak tile could name different techs (109 Daniel vs 108 Jason) because the ticker is the season MAXIMUM while the tile is an AWARD SEAT that `assignAwards` may give to a credible runner-up. Both were right; the label made them look contradictory. Reworded to **"Team best"** on both boards — the narrow board had no label at all, which was worse — and the value now computes over ALL techs (Cesar included, streak 86 today) since it's a numeric season stat and exclusion is awards-only. Echoing the tile was rejected: it would print "longest streak 108" while a 109 exists. rev 39 — **RESPRAY RULE CORRECTED: the ~9-day window is a BOOKING rule, not a completion rule** (§5.12, ops-clarified; CORRECTS rev 38). Rev 38 excluded 39 re-services (39%) as "anomalies" for completing late — wrong: the office applies the ≤9-10 day test at BOOKING, and rain/capacity push the visit out. **Every completed re-service with a prior spray is now attributed again — resprays 60 → 99, team rate 1.11% → 1.84%**; long gaps get an informational "late visit" marker only. **Maturity is now 3 conditions** (`MATURITY_DAYS` 9 → 10): 10+ days elapsed, no completed re-service, AND **no re-service on the books** — a booking made on day 9 can be performed on day 15, so quiet days alone can credit an already-broken streak. Pending detection probed on `/scheduled-services` (Type column carries "Re-service", Status "Pending"; rare — ~1 in 166); **measured cost 385 ms/customer, pooling does NOT help**, so scoped to customers whose MOST RECENT spray is 8-21 days old = **514 customers / 198 s** as a nightly cron phase writing `mosquito_service_status.pending_reservice`. Matured clock moves to **Jun 28-Jul 3** (volume clock stays Jul 12-17). Board footers reworded. Cesar now reads **500/9 = 1.80%**; ticker **5,393 · 1.84%**. rev 38 — **RESPRAY WINDOW (9d) + MATURITY + TWO CLOCKS + awards-only exclusion** (§5.12/§5.15/§5.16, ops-confirmed). **(1)** A respray is now a re-service **within 9 days** of the prior spray, blamed on that spray's tech — supersedes rev 24's windowless rule. Beyond 9 days = **anomaly**: counted in the total, blamed on nobody, listed for review. **Live: attributed resprays 99 → 60, team rate 1.84% → 1.11%, 39 anomalies.** ⚠️ The gap data shows NO clean cliff (mode 8-9d but a real tail to 20d; ≤9d covers 61%, ≤12d 84%, ≤14d 90%) — 39% called "anomalies" is a strong claim, and `RESPRAY_WINDOW_DAYS` is a one-constant change if ops wants the tail. **(2) Maturity:** proven clean = 9 days elapsed with no re-service; clean streaks count proven sprays only. **(3) TWO CLOCKS** — VOLUME (Workhorse/Road Warrior/spray counts) = last completed Sun-Fri week (**Jul 12-17**); MATURED (Iron Wall/Perfect Week/Most Improved/rates/Clean Streak) = most recent week where every spray is 9+ days old (**Jul 5-10**). **Every award tile now prints its own date range**, plus a footer on both boards stating the respray rule. **(4) Exclusion is AWARDS-ONLY**: Cesar + `Z-*` win nothing but count in every number — **the YTD ticker now reads 5,393 sprays, not 4,892**. Probed: Z-ASAP/Z-RLW are real completed work, not routing artifacts. **(5) NEW season-pace card:** 5,393 vs 5,185 by the same date last year (+4.0%). rev 37 — **NEW "Cadence health" stat card on `/service/resprays`** (§5.12). Share of gaps between a customer's consecutive mosquito services that ran **strictly beyond 17 days** (the top of the 11–17 day target window): **2024 9.1% (1,070/11,705) · 2025 27.8% (2,643/9,514) · 2026 LIVE 31.1% (1,331/4,282)** — roughly **3.4× in three seasons, +22.0pp vs 2024**, and the live season is tinted amber with the delta. Turns the §5.17 spray-gap finding into a tracked leading indicator for resprays and retention. ⚠️ **`>17`, not `>=17`** — 17 days is ON TARGET; the earlier ad-hoc ">17" figures (12.3%/35.4%/38.7%) actually included exactly-17-day gaps. `CADENCE_WINDOW = {min:11,max:17}` in the new `lib/service/cadence.ts` is the single definition. Current season is free (computed from the job rows the report already holds in memory); the two completed seasons are `LAG` aggregates in Postgres, not 25k rows pulled into the request. rev 36 — **SPRAY WEEK IS NOW SUNDAY–FRIDAY** (§5.15/§5.16, ops). The crew sprays Sun–Fri and never Saturday, so `resprays.ts::weekStart()` returns the **ISO Sunday** (was Monday) and every consumer moved with it — tech board landscape + tall, `/service/resprays` weekly leaderboard, Most Improved week-over-week, all week bucketing. Bucket stays **Sun–Sat (7 days) with a structurally empty Saturday** so a stray Saturday job can't vanish; **displays show Sun–Fri** (`weekEnd = start + 5`). **ROLLOVER: the board flips on SATURDAY** — `boardWeekStart()` = the most recent Sun-start week whose Friday has fully ended (Fri Jul 17 → Jul 5 · **Sat Jul 18 → Jul 12** · Sun Jul 19 → Jul 12 · Sat Jul 25 → Jul 19); nothing can land after a week's Friday, so waiting for Sunday would only show a staler week. ⚠️ `categorize.ts::startOfSaturdayWeek` is a SEPARATE **Sat–Fri** sales-snapshot week — left alone on purpose. ⚠️ Pre-rev-36 `tv_tech_awards` rows are Monday-keyed, so repeat-avoidance no-ops for one transition week, then self-heals. New `scripts/verify-week-tieout.ts` reconciles the board against the **live** Pocomos report (not the mosquito-only cache): Nathaniel, Jul 12–17 = **146 rows → 115 counted + 31 excluded** (12 Add-On Tick, 9 Other, 7 Natural Add-On Tick, 2 Re-service, 1 Other Initial), board count 115 ✓, cache 117 = 115 apps + 2 re-services ✓, Sat/Sun both 0. rev 35 — **TV EMOJI → inline SVG icons** (§5.16). **⚠️ Yodeck's Linux browser has NO color-emoji font** — every award/weather/💧 glyph rendered as an empty box on the real screens while looking perfect on every dev machine. New `components/tv-icons.tsx` (lucide-react): `AwardIcon` in a gradient badge disc with a soft glow and a per-award accent (Clean Streak pink · Iron Wall blue · Workhorse amber · Road Warrior green · Most Improved teal · Perfect Week gold), `WeatherIcon` (WMO code → sun/cloud/rain/storm, tinted, unbadged so it doesn't fight the award wall), `PrecipIcon`. **Emoji fields DELETED from the data layer** — `AwardDef.emoji` gone, `ForecastDay.emoji` → `ForecastDay.code` — so there is no emoji left to render. **Icon colors are INLINE styles**, never dynamic Tailwind classes (those purge and render colorless in prod — the same invisible-in-production failure as the emoji). New `scripts/verify-tv-icons.ts`: zero emoji codepoints in rendered DOM text AND in `::before`/`::after` content, SVG counts, no clipping, at 1080p + all four tall sizes, with screenshots to review. rev 34 — **FIX: fleet-count CSV endpoints 404'd on Vercel, breaking Sheets IMPORTDATA** (§5.14). **⚠️ LANDMINE: an App Router route segment containing a DOT compiles, lands in the build manifest, and serves 200 under `next start` — but 404s on Vercel for EVERY client**, because the platform resolves extension-looking paths against the static filesystem and never dispatches to the function. Not the user agent, not middleware, not a redirect. Handlers moved to dot-free paths (`/api/fleet-counts/{both,customers,services}`) with `beforeFiles` **rewrites** preserving the public `.csv` URLs unchanged — rewrites not redirects, so sheets see 200 + `text/csv` with no hop. **Never create a route directory with a dot in its name.** rev 33 — **2021-2023 RealGreen history loaded + 5-SEASON return-rate trend** (§5.17). The return rate goes from 2 points to **five**: **82.3% (21→22) · 80.7% · 79.6% · 78.9% · 77.3% (25→26)** — a **−5.0pp monotonic decline**, while denominators GREW (1,061 → 1,296), so it's retention sliding, not a shrinking sample. New `realgreen_jobs_history` (year-keyed) + `return_rate_history` (frozen pairs); `realgreen_jobs_2024` deliberately left alone so the anomalies worklist stays scoped to years ops reconciles. **⚠️ LANDMINE: history is computed in RealGreen SHORT-ID space, NEVER through `customer_id_map`** — measured on 23→24, the map drops **232 of 1,236** denominator customers and inflates the rate **79.61% → 96.91% (+17.3pp)**, because the ids it can't resolve are exactly the churned customers a denominator is made of. **Comparability seam quantified:** historical pairs are spray-only (no Pocomos tags existed); on 24→25, computable both ways, spray-only reads **76.93% vs 78.92% ≈ 1.9pp** — so the UI **encodes** the seam (dashed span + hollow markers + `spray-only` row tag + footnote) rather than hiding it. New `scripts/load-history.ts` (7.9s, idempotent; reads 2024 from the loaded table, not the CSV) + `scripts/verify-return-trend.ts`. CSV quirks: single `\r` endings, `0:00` times, and a blank comma-only row between every data row (skipped explicitly, else a clean load looks 50% broken). `MOSQUITO_SERVICE_TYPES` is lowercased — compare `LOWER(agreement)` or you get 0 rows. rev 32 — **TV nav dropdown** (§5.14). Fourth nav dropdown **TV** → Sales board (`/tv/sales`) · Tech board (`/tv/techs`) · Tech board — narrow (`/tv/techs/tall`), so the Yodeck boards are reachable from the dashboard instead of by typed URL. **Items open in a NEW TAB** — `Shell` renders `/tv/*` without the nav, so an in-tab link strands the user on a kiosk page. New opt-in `NavLink.matchPrefix` (`"/tv"`) lights the tab across `/tv/*` while its landing href is `/tv/sales`; opt-in so **Sales doesn't** inherit it and double-highlight with the Finance tab (`/finance` is a Sales child AND its own tab). New `scripts/verify-nav-tv.ts` (click-to-open, hrefs, `_blank`+`noopener`, Escape-closes, no-double-highlight). rev 31 — **TV-TECHS-TALL: rotation REMOVED, all six awards always visible** (§5.16). Rev 30's 3-up rotation solved the wrong problem — the tiles were small because the grid was **one column**, not because six don't fit. New `columnsFor(w,h)`: tall-and-narrow (`h>w && w<700`) → 1 col; wide (`≥1000`) → 3 col; **everything else → 2 col**, so the real ~470×430 Yodeck slot renders a **2×3 wall with 222×89px tiles** (vs rev 30's ~44px rows) and nobody waits 15s for the other half of the board. Tile type now scales off the **measured tile BOX** — `min(tileH×0.20, tileW×0.125)`, clamped 8–34px: height sets the scale, **width caps it** so a long stat ("75 sprays, 0 resprays") can't truncate on a squat 2-up tile. Verify script rewritten to assert **all six labels + the measured column count** at every size (470×430 → 2×3 · 550×700 → 2×3 · 600×900 → 1×6 · 1200×600 → 3×2, 0 clipped). Landscape `/tv/techs` unchanged. rev 30 — **TV-TECHS-TALL award TILES + adaptive rotation** (§5.16). `/tv/techs/tall` awards restored to TILES matching the landscape board (uppercase colored LABEL + emoji, big first name, stat), single column at Yodeck widths, 2 columns only ≥900px. **Adaptive:** the awards region measures itself — all six tiles when each row clears 76px (550×700, 600×900, 1200×600), else **rotates 3 at a time with a 15s cross-fade** (the real ~470×430 slot). **Tile type is `em` off the MEASURED row height, not viewport units** — vw/vmin type can't know its row height and clipped names mid-glyph at 550×700 while the DOCUMENT fit fine; verify now asserts **per-tile** `scrollHeight ≤ clientHeight`. **⚠️ Verify `/tv/*` against a production build** — `next dev` served CSS missing `h-screen`/`flex-1`/`grid-cols-4` entirely, and a zombie server on a held port serves the old bundle; both produced phantom failures. Landscape `/tv/techs` unchanged. rev 29 — **TV-TECHS-TALL: `/tv/techs/tall`** (§5.16). Narrow companion to the landscape board for the RIGHT COLUMN of the Yodeck layout; widget URL `https://ms-operations-hub.vercel.app/tv/techs/tall`. Same `getTechBoard()` data and rules, **no new scrape/cron**; landscape `/tv/techs` unchanged. Weather strip · header · six one-line award rows (emoji + first name + stat) · YTD ticker; **no bottom table**. **Responsive with no breakpoints** — every size is `clamp(min, Nvw, max)` because the Yodeck widget IS the viewport; verified 500×450 / 550×700 / 600×900 with zero overflow. Weather = **Open-Meteo, free, no key** (probed: parallel `daily` arrays; 40.61/-73.71 snaps to 40.597/-73.702), today + next 3, **display-only**. Its 30-min TTL is a **module memo, not Next's fetch cache** — the page's `force-no-store` would bypass the latter; **fail-soft** to a stale memo or `null` so weather can never take down the board. New shared `useAutoReload` hook. rev 28 — **TV-TECHS: `/tv/techs` shop-TV "Tech Board"** (§5.15). Yodeck webpage-widget URL `https://ms-operations-hub.vercel.app/tv/techs` — 1080p, self-reloads every 10 min, no interaction. **No new scrape/cron**: computed on read from `respray_jobs` + `mosquito_service_status` (route join `pocomos_id = customer_id`, probed 99.7% coverage / 44 routes). **Board week = last COMPLETED Mon–Sun week** — the live week had 609 sprays and 0 resprays (resprays lag the spray they're blamed on), which would make Iron Wall / Most Improved / Perfect Week meaningless; the clean-streak walk is capped at week end for the same reason. Six awards (🎯 Clean Streak · 🛡️ Iron Wall · ⚡ Workhorse · 🗺️ Road Warrior · 📈 Most Improved · 🏅 Perfect Week), Cesar Barrerra excluded, **no negative callouts**, route-fair (volume is its own award, never a quality judgement). **Every tech gets ≥1 award via maximum bipartite matching (Kuhn's)** — greedy produced "Clean Streak — 2 in a row" over a real 138-streak; candidate floors + descriptive (non-superlative) blurbs keep every displayed stat true. New table `tv_tech_awards` (repeat-avoidance only). Referral trophy DEFERRED — `AwardDef.topBilling`/`.spin` hooks reserved (BACKLOG TV-TECHS-REFERRAL). rev 27 — **BUILD-SPEEDUP tooling** (§13). No app behavior change. **(1) Playwright MCP** configured in `.mcp.json` (project scope — future sessions inherit; interactive UI verification). **(2) `scripts/lib/`** shared helpers: `livecheck.ts` (Playwright live-page check — the curl+regex replacement), `pocomos.ts` (`legacyDataTablesBody` + session re-exports), `csv.ts` (`writeCsv`/`readCsv`, handles the `\r\r` export quirk), `neon.ts` (`sql` re-export). New `scripts/verify-live.ts` smoke test. Refactored `probe-roster-reconcile.ts`, `list-renewed-no-2025-season.ts` onto the lib; **deleted 44 obsolete one-off scripts** (71 → 27 + 4 lib). **(3) Skills**: `.claude/skills/pocomos-scraping` (surfaces, DataTables 1.9, id conversion, landmines) + `.claude/skills/dashboard-conventions` (cron+cache pattern, shared components, idioms). CLAUDE.md slimmed to identity + always-true rules + pointers. **(4) `/ship` command** (`.claude/commands/ship.md`) encodes the ritual. **(5) Vercel:** repo IS git-connected (`vercel git connect` confirms) but auto-deploy is intermittent; needs a dashboard check — until fixed, `/ship` runs `npx vercel --prod`. **(6) Rule: verify deployed UI via Playwright, NOT curl+regex** (en-dash / attribute-order / dropdown-children-on-click false-FAIL history). rev 26 — **RL-04 follow-up: CLOSED-OUT bucket + collapsible sections** (§5.11). The old `loop_not_closed` (234) hid the spec's CLOSED-OUT bucket: split into **closed_out 185** (a task was completed, none in progress — done reaching out; new section shows closing description, completion date, salesperson, Not-Interested reason, lead link) and **loop_not_closed 49** (reached but no task ever completed / none open). Completion date parsed from the archived task's plain-text date (`parseUsShortDate` — history rows have no `data-order`). **UI reworked to collapsible per-bucket sections** (anomalies-card pattern; never_reached + loop_not_closed open by default) for an obvious one-click view of each category. Live: never_reached 43 · loop_not_closed 49 · closed_out 185 · working_overdue 6 · working_on_track 7. rev 25 — **RL (Mrs. L) feedback pack: 3 refinements**. **UPDATE-RL-01 — duplicate detection** (§5.10): (a) records whose name contains "duplicate" are dropped from detection (known shells); (b) a shared email is no longer proof of the same person — a dup group now requires **name identity** (same last name + fuzzy first name), so fathers/daughters sharing an inbox aren't flagged. **Dup groups 83 → 59** (24 removed: 25 "…duplicate…" shell records across ~15 groups + 9 different-people name mismatches). **UPDATE-RL-03 — "Needs manual check"** (§5.5): when a customer's profile opens to a non-mosquito contract, we now **resolve their last mosquito service from the completed-jobs report (`respray_jobs`)** instead of flagging — the mosquito contract is already identified (they're eligible) and the completed-jobs cache is contract-specific. **needs_check 7 → 0** (6 → current, 1 → overdue; Chana Lovi + Susan Badalbayev cleared without the Monday tag fix). `refreshMosquitoStatus` now refreshes `respray_jobs` at the TOP (was before the ASAP phase). **UPDATE-RL-04 — `/leads/followup` reclassification** (§5.11): ops model = tasks and notes are SEPARATE. **Added a Lead Notes scrape** (`/lead/{id}/lead-information` `#notes-table`, `data-order` dates; the bulk `/all-notes-data` feed is customer-only, no lead notes). New buckets replace the old 4: **never_reached** (no task ever AND no notes) · **loop_not_closed** (reached but no in-progress task) · **working_on_track** (in-progress task, due today/future) · **working_overdue** (in-progress task, past due). **Live: never_reached 43 · loop_not_closed 234 · working_overdue 6 · working_on_track 7** (290 scope; 290 boards + 290 note pages, 175s). rev 24 — **respray pack: rule change + weekly leaderboard + chain/repeat** (§5.12). **(1) RESPRAY-RULE-CHANGE (ops-canonical):** a respray is now **ANY completed mosquito re-service this year** — the rev-21 **10-day window is GONE**. Attribution blames the tech of the customer's **most recent prior mosquito job**, where prior jobs **include prior re-services** (CHAIN rule: re-service-of-a-re-service blames the last toucher). Denominator unchanged (Initial+Regular apps). **Live 2026 YTD: 99 re-service jobs, all 99 attributed (0 unattributed), 3 chain, team rate 1.84%** (was 1.30% under ≤10d; +29 resprays team-wide as the 28 previously-excluded 11+ day jobs + chain now count). Only **Nicholas Rosales flagged** (3.12%, 1.70×, 1,059 apps). **(2) RESPRAY-WEEKLY:** a "This week on the board 🏆" leaderboard section — current + last full week, per-tech sprays/resprays/rate, Best (lowest rate) + Watch (highest rate) callouts (20+ apps that week), plus auto-stats (zero-respray streaks, most-improved, perfect-week). **(3) RESPRAY-CHAIN:** drill-down rows badge a **"chain"** respray (blamed prior job was itself a re-service) and show the prior job's type; new **"Repeat respray customers"** card lists accounts with 2+ resprays (count, chain count, profile link) — live 8 customers. rev 23 — **nav dropdowns + `/finance` + public fleet-counts feed** (§5.14). **(1)** Top-nav **Service** and **Sales** are now click-driven dropdowns (same pattern/component as Leads — `NavDropdown`, works on touch, tab highlights on all child pages): Service → "Overdue sprays" (`/service/overdue`) + "Respray performance" (`/service/resprays`); Sales → "Sales" (`/sales`) + "Paused — open balance" (→ `/finance`). New top-level **Finance** tab. **(2)** New **`/finance`** page hosting the "Service paused — open balance" roster via a **shared `PausedBalanceCard`** (extracted with `RowTable` into `components/service-rows.tsx`; `/service/overdue` renders the SAME component in place — no copied code). Built to grow (payment-retry review lands here next). **(3)** Public read-only **`/api/fleet-counts`** (JSON) + **`/api/fleet-counts.csv`** (Sheets IMPORTDATA) — totals only, no names, derived live from the nightly `mosquito_service_status` table. **customer_total = 1,156** (eligible mosquito customers = the overdue page's "Eligible" total); **service_total = 1,175** (each customer once, Weekly-cadence contracts twice — 19 weekly; a van-capacity gauge, ≈4.7 vans at 250/2wk). rev 22 — **`/service/overdue` "sprayed today but shows overdue" bug FIXED** (§5.5). Routes 109/209 (and others) showed customers overdue on the very day they were sprayed. **Two root causes:** (1) the overdue cache's last-spray date lags — a tech's completion syncs to Pocomos's bulk **"Last Service"** field hours later, so a daytime refresh still reads the old date; (2) the bulk **`next_service_date`** is a stale PAST slot Pocomos never advances (e.g. Cynthia Kotowitz 1163433 sprayed today, next reads 07-10), so the existing **scheduled-today rule (`next == today`) can't catch them**. **Fix:** new **sprayed-today read-time rescue** — `getOverdueReport` cross-checks the completed-jobs cache (`respray_jobs.completed_date = easternToday`, mosquito-only) and pulls those rows into a green **"Sprayed today"** card, excluded from the overdue count, with a "Sprayed" pill. **Precedence sprayed > scheduled > ASAP > overdue.** `refreshMosquitoStatus` now refreshes `respray_jobs` (one ~3s POST, guarded) so "Refresh now" reflects same-day sprays. **Verified: all 8 overdue rows sprayed 2026-07-17 (routes 209×4/109×3/309×1) moved to Sprayed-today, 0 left wrongly overdue.** rev 21 — **NEW `/service/resprays` "Tech Respray Performance"** (§5.12) + **`/leads` staleness fixed** (§5.13) + Leads nav dropdown. **(1)** Respray rate by tech, sourced from the Pocomos **completed-jobs report** — a Symfony form POSTing to itself that renders `#results-table` server-side; **ONE POST returns the whole year** (6,246 rows, 5.4 MB, **3.2s**, no per-customer scrape) and every row carries the customer web id. Cached in **`respray_jobs`** (all 5,391 mosquito jobs); attribution computed on read. Cron `0 8 * * *` + Refresh-now. **Ops attribution rules** (on the card): CY only; a `Re-service` is a respray **only ≤10 days** after that customer's prior mosquito application (Initial/Regular), attributed to **that prior spray's tech**; **11+ day gaps are normal cadence (11–17d) and are NOT counted**; no prior spray this year = unattributed; rate = attributed resprays ÷ his applications YTD. **Live 2026 YTD: 97 re-service jobs · 69 counted (≤10d) · 28 excluded (11+d) · 0 unattributed · 5,294 applications · team rate 1.30%.** Only **Nicholas Rosales flagged** (2.59%, **1.99×** team, 1,041 apps); flag = ≥1.5× team avg AND ≥30 apps. Weekly breakdown buckets a respray into the week of **the spray it followed**. **(2) `/leads` "Updated 749h ago" root cause: the close-rate cache had NO cron** — `refreshCloseRate` only ran from the manual Refresh button or on GET when the row was *missing*, so a stale-but-present row lived forever. **NOT conversion-sweep** (that only moves PhoneBurner folders). Fixed with cron `/api/cron/close-rate` `0 9 * * *`; refreshed live (424 leads · 113 conversions · 26.7%). **(3)** Leads nav is now a **click-driven dropdown** (Close rate / Follow-ups) — click, not hover, so it works on touch; the Leads tab highlights on any `/leads/*` page. `CollapsibleSection` extracted to `components/ui/collapsible-section.tsx` and shared by /sales + /service/resprays. rev 20 — **NEW `/leads/followup` "Overdue Follow-ups" page** (§5.11). Open leads created THIS YEAR (`status=Lead` + `date_added` in CY — **288 live**), classified by the state of their follow-up TASK. Nightly cron `0 7 * * *` → `leads_followup` cache + "Refresh now"; page never scrapes on load. **Probe results:** `POST /leads/data` already returns status + `reason`/`reason_name` + marketing type, so **no per-lead `lead-information` scrape is needed**; the task trail is on the **server-rendered** `/lead/{id}/message-board` (`#todo-table` = open, `#history-todo-table` = archived; due cell has `data-order="YYYY-MM-DD HH:MM"`; `<span class="comments-count">` = touch count), and comment TIMESTAMPS need one extra GET per commented task (`/message/todo/{id}/show` → `.comment__date`). **Measured cost: 288 + 128 = ~416 GETs in ~229s at concurrency 5, 0 failures.** **4 buckets, not 3** — the spec's three didn't cover 185 leads whose every task is Completed with nothing rescheduled, so **"No open task"** was added: **Overdue 1 · No task 92 · No open task 185 · On track 10**. The finding: the team *completes* tasks rather than letting them lapse, so Overdue is nearly empty while **277/288 open 2026 leads have no scheduled next step**. **PhoneBurner cross-ref is thin:** `webhook_log.pocomos_id` is NULL on all 293 rows — must bridge `pb_contact_id → phoneburner_contacts → pocomos_id`; only 5/288 scope leads have any call event (136/288 are synced to PB at all). ⚠ `POST /todos/{id}/complete` (Mark as Completed) and `/message/todo/new` sit right beside the scraped data and are MUTATIONS — never touch. rev 19 — **"returned" rule widened + season buckets partition again + new anomalies card** (§5.8, §5.10). **(1) Rule 2 rewritten**: a customer RETURNED in Y+1 if they're **ACTIVE now with ANY `{Y+1} -` tag** (*signing up IS returning* — sprays not required; **any** tag, so `New Sale` re-signups finally count) **OR** they meet the **Y+1 spray rule regardless of current status** (credits sprayed-then-churned). Applies to **both pairs** — rev 18's in-progress-only restriction on the tag path is LIFTED. Denominator untouched (rule 1, sprays only, any year). **(2) `24→25 = 78.8% (1,006/1,276)`**, up from 77.8% (+1.0pp; 808 by tag / 198 by sprays — the whole move is the tag path now applying to a completed season). **`25→26 = 77.3% (949/1,227)`**, +1 customer vs rev 18's 948 (4 `New Sale` re-signups added; 3 moved spray→tag). Returning box **949** (Auto 388 · SEB 287 · EB 138 · Renewed 115 · New Sale 4 · spray/other 17). **(3) Season buckets PARTITION the active roster again** (`SeasonBuckets`): **New 150 + Season-Skipped 86 + Returning(active) 932 = 1,168 = Active Customers** ✓ — an active `{CY}`-tagged customer who wasn't a real CY-1 customer lands in New (no history) or Season-Skipped (prior tag *or* prior spray). All three /sales + /tv/sales tiles now read the taxonomy, not `categorize.ts`. Returning still displays the full box (949) — the 17 `churnedReturners` are named in the reconciliation line. **(4) NEW "Return-rate anomalies" card** (§5.10): live, self-clearing roster of records we can't measure cleanly, with per-class stat header, reasons and profile links — **117 today**: 83 duplicate records · 26 unmapped export ids · 8 unreadable histories · 0 sprayed-without-tag. Measurement faults only; tag hygiene stays in the Missing-tags card. `CONTINUATION_TAGS_NAMED` no longer gates the rule — it only drives the box sub-counts. rev 18 — **return-rate counts now come from AUTHORITATIVE BULK JOB EXPORTS; 24→25 UNBLOCKED** (§5.9, §5.8). **(1)** Completed seasons are loaded from job-level exports — **2024 from a RealGreen dump** (received + loaded; the pre-Pocomos era) and **2025 from a Pocomos completed-jobs export** — into `realgreen_jobs_2024` / `completed_jobs_2025`; only the in-progress CY is still scraped (`scrapedYears()` = `[CY]`). This retires BOTH blockers at once: the pre-Pocomos 2024 gap *and* the scrape's **contract-scoped blind spot** (the service-history page renders only a customer's DEFAULT contract, so a season on a cancelled contract read as a phantom zero — Sherly Aminzadeh 1234543: scrape said 2025 = **0**, export says **6**). **(2) `24→25 = 77.8% (993/1,276)` is LIVE** (was `n/a` since rev 15; `reliable` now keys off the from-year being export-backed). **(3) `25→26 = 77.3% (948/1,227)`, up from rev 17's 75.9% (976/1,286) = +1.4pp** — the scrape denominator both missed blind-spot customers and included customers with no real 2025 season. Returning box = **948**, still === the numerator. **(4)** New `mosquito_service_counts.source` column (`'export'|'scrape'`) + the **invariant that an export-backed year holds ONLY export rows** (stale scrape rows are evicted: 28 in 2024, 96 in 2025); the nightly scrape now writes/prunes `year=CY AND source='scrape'` only. **(5)** The **tag path is now IN-PROGRESS-season only** — a completed season's spray record is final, so a continuation tag without sprays no longer counts (24→25 numerator: 0 by tag, 993 by sprays). **(6)** `short_id → web id` map **BUILT from contact details** (`customer_id_map`, `lib/service/idMap.ts`) because Pocomos exposes the short id nowhere — 1,609/~1,635 mapped, ~26 unresolved fail closed (≈1% of jobs). **(7)** RealGreen code mapping **validated empirically** (818 both-year customers): `12`→Mosquito 98.8%, `12N`→Natural 98.1%, `24`→Mosquito-Weekly 100%, `24N`→Natural-Weekly 100% — all four mosquito (visit-count programs, N = Natural); a ratio guess of `24`=Tick was WRONG. **(8)** §5.8 now records two landmines: the per-contract **PDF is never accurate** (ops ruling; it's an invoice packet — mismatched the table 4/5) and **`POST .../service-history/{paid,unpaid}` are ASYNC ACTIONS, not feeds** (405-on-GET is an action signal). Known artifact: duplicate web records (one human, 2+ ids) cost the numerator **exactly 3** (+0.2pp) — in BACKLOG. rev 17 — **return-rate + "Returning" box unified** (§5.8), year-relative throughout. **(1) The "real customer of year Y" rule is REVERSED from rev 16**: real = **≥2** completed mosquito services in Y, **OR exactly 1 dated AFTER `LATE_SEASON_CUTOFF` (Aug 15)** — a late-season signup who joined too late to have had a second spray. A single **early/mid-season** spray is now a one-off and does **NOT** count (rev 16 had it backwards: it excluded the single *late* spray and accepted the single early one). Constant `REAL_CUSTOMER_MIN_SERVICES` added. **(2) "Returned in Y+1" is now a COMBINED test**: real customer of Y+1 **OR** an **Active** customer holding a Y+1 continuation tag (Auto/SEB/EB/Renewed — `CONTINUATION_TAGS_NAMED`); the **denominator stays rule-1-only** (no tag path). **(3) The `/sales` + `/tv/sales` "Returning" box IS the numerator set** (`taxonomy.returningBox`), restricted to prior-year real customers — the two cards can no longer disagree (`box.total === pair.returned`, asserted by `scripts/verify-return-unification.ts`). Auto/SEB/EB/Renewed sub-counts kept, **new `bySprayHistory` sub-count** (qualified on sprays with no continuation tag); sub-counts partition the total. **(4)** Card description + the wrong "excluding late one-offs" label rewritten to the new rule. `sales-provider.ts`/`summarize()` deliberately **untouched** — `buckets.RETAINED` + `retainedSubtypes` stay the tag-only series feeding `snapshots` (historical continuity), just no longer displayed; the on-screen identity `Active = NEW + RETURNING + RETAINED` therefore no longer holds. **Live 25→26 = 75.9% (976 / 1,286)** vs the rev-16 rule's **76.5% shipped / 76.6% (946/1,235) recomputed today** = **−0.7pp**; numerator paths 954 tag + 22 spray; late-season signups counted real: 2025 = 88, 2026 = 0. **Returning box = 976** (Auto 404 · SEB 292 · EB 139 · Renewed 119 · by spray history 22), was 1,009 tag-only — verified diff: 55 dropped (all denominator-membership; 7 of them `table_ok=false` coverage), 22 added (spray-history qualifiers, 19 non-active). **(5) HISTORY: Pocomos data starts 2025 — the company used RealGreen before that**; 2024 service history will arrive as a RealGreen dump, so 24→25 stays `n/a` (see BACKLOG). rev 16 — two changes, both year-relative (all logic derived from `CURRENT_YEAR`, never hardcoded). **(1) Return-rate "real customer" rule changed** (§5.8): replaces the `MIN_RETURN_TREATMENTS=2` threshold. A **real customer of year Y** = received **≥1** completed mosquito-family service in Y (Event Spray never counts), **EXCEPT** a customer whose **only** Y spray landed **after `LATE_SEASON_CUTOFF` (Aug 15)** — a late one-off (extended-season sale), not a real customer. Same test on both denominator (year Y) and numerator (Y+1). Per-spray dates added to the counts cache (`mosquito_service_counts.first_service_date` / `last_service_date`) so the carve-out is computable; `LATE_SEASON_CUTOFF` is a documented month/day constant in `sales-taxonomy.ts`. **Live 25→26 = 76.5% (945 / 1,235)** vs the old ≥2 rule's 75.9% (rev 15) / 76.2% recomputed today. **Single-late-spray customers excluded: 2025 = 89, 2026 = 0** (today is before Aug 15, so no 2026 late sprays exist yet). 24→25 still **n/a** (service-history truncation — `reliable=false`). **(2) "Missing tags" section on `/sales`** (§3.5): ALL currently-active customers with **no `{CURRENT_YEAR}` tag** (any prior tags or none) — name, id, all tags, last service date, Profile link (new tab), with a small stat header. **Supersedes/absorbs the narrower "Customers with issues"** card (active + no current AND no prior tag): Missing tags is the full off-bucket-active superset, and the old issues subset is now flagged inline with a "no prior tag" badge. **Live: 10 missing-tag customers (8 not-renewed-with-prior-tag + 2 no-prior-tag).** rev 15 — three changes. **(1) `/service/overdue` ASAP-route rescue** (§5.5): an overdue account with an upcoming job assigned to the **"Z-ASAP" route** (probe-confirmed: appears as Technician `Z-ASAP 01` + Route Assigned `Assigned` on `scheduled-services`, detected per-row) is being caught up → pulled into its own blue **"On ASAP route"** card with an ASAP pill, excluded from the count (new "Excludes N on ASAP route" sub-line), cached in `mosquito_service_status.asap_route`, scraped ONLY for currently-overdue rows (cheap — 17 rows/6.5s). **(2)** the green **"Scheduled today" section moved BELOW** the overdue table. **(3) Return rate REDEFINED to completed-service counts** (§5.8): ≥`MIN_RETURN_TREATMENTS`(=2) completed mosquito services (Event Spray never counts) in Y AND Y+1. New resumable nightly scrape `serviceCounts.ts` → `mosquito_service_counts` + `mosquito_service_scrape` (cron `/api/cron/service-counts`), card shows "(computing — N% covered)". **Live: 25→26 = 75.9% (911/1,200), 100% covered.** **24→25 = n/a** — the Pocomos service-history table is truncated to ~1 recent season, so 2024 counts collapse; needs a full-history source (PDF export). rev 14 — **return-rate redefined per ops as completed-service-based** (§5.8): the metric is now "received ≥1 completed mosquito service in year Y, and receiving service in Y+1?" A new `servedInYear()` predicate (`sales-taxonomy.ts`) replaces the tag-only test — CURRENT season requires the customer to be LIVE (fixes the rev-12/13 bug where 123 cancelled auto-renew-tagged customers were counted as returns), past seasons use `last_service_date` evidence with the season tag as a continuity fallback. Dual primary/excl-mid-season denominator removed (one rate per pair); §9 open #8 resolved. **Live: 24→25 = 80.5% (1,037/1,288, tag-anchored est.), 25→26 = 72.6% (956/1,317, live-status-anchored).** On-Hold counts as returned (±0.1–0.3pp). Task-#2 reconciliation: 55 Returning-bucket customers not in the numerator are all correctly excluded (51 have no 2025 mosquito tag). rev 13 — follow-up fixes. `/service/overdue`: scheduled-today rows now get their **own green "Scheduled today" section** (removed from the overdue table); **sticky headers fixed** (the `overflow-x-auto` wrapper was the sticky scroll-context — removed it, solid-bg + z-20 header now pins on page scroll); **Route column added** — the route CODE turned out to EXIST on the `service-information` "Routing" widget (field "Code", e.g. `510`/`RI1`), reversing rev 12's "no route code" finding. Scraped into a new `mosquito_service_status.route_code` during the daily refresh (incremental — only `route_code IS NULL` rows; `?forceRoutes=1` re-scrapes all); full backfill of ~1,022 rows added **~315s** to the refresh, ~0 steady-state. Return-rate: added an audit (`scripts/audit-return-gap.ts`) — the 25→26 numerator (1,079) exceeds the "Returning" bucket (1,004) mostly because it counts **128 now-inactive/on-hold customers** carrying a 2026 continuation tag (see §5.8); added a card hint + a Needs-a-decision item. rev 12 — shipped two builds. `/service/overdue`: **scheduled-today rescue** (a row whose next service is today (Eastern) is tinted green, tagged "Today", and dropped from the overdue COUNT with an "Excludes N scheduled for today" sub-line — computed at read time, see §5.5) + **sticky table headers**. `/sales`: **return-rate card** (24→25 = 73.4%, 25→26 = 74.0% live) computed in `getSalesTaxonomy` — real mosquito customers (mosquito-family contract carrying that season's tag, Event-Spray-only excluded) who returned the next season, with a primary and an excl-mid-season denominator (§5.8); compact on `/tv/sales`. Route column for `/service/overdue` **deferred** — no route CODE exists in any Pocomos source (§9 open). rev 11 — full truth-up, doc now matches deployed code as of 2026-07-07. Audited against the live source tree + a live Neon `information_schema` query: added the "Current live state" block; §5 marks the PhoneBurner sync + webhook SHIPPED/LIVE since 2026-05-15 with real file paths + the four live crons; §6 file tree replaced with the actual tree; §7 lists the nine tables that really exist now (adds `leads_close_rate`, real row counts, `webhook_log.pb_contact_id`); §9 resolved items pruned and the lead-data path documented as the working answer (Advanced Search two-step feed — lead *tag-chip* read is the only lead gap left); §12 lead notes corrected. NOTE: requested as "rev 6", but the doc was already at rev 10 — advanced to rev 11 to preserve the rev 6–10 history below rather than regress the number. rev 10 — synced doc to shipped reality: the `/texting` Aerialink archive + the app's only auth gate (§5.7), the hourly roster-reconciliation conversion sweep (§5.5b), the `/leads` close-rate tab (§5.6), and the `texting_messages`/`texting_contacts` tables (§7) are all LIVE. Texting search is now server-authoritative via `?find=` (commit `cdf1b3f`); §6 file tree now lists the texting routes + `middleware.ts`. rev 9 — made `/sales` and `/tv/sales` snapshot-first: they paint instantly from the latest snapshot's `raw_json` (a fast DB read) and revalidate live in the background via a new `GET /api/sales/live` + `useLiveSales()` hook; label flips "as of {date}" → "live · updated just now". `normalizeSummary()` defends against partial/older snapshots; empty-table falls back to a live build. Page-level `AutoRefresh` removed (polling is now client-side). See §3.5 "Load path". rev 8 — rolled the `/sales` contract-type breakdown up into service families in a fixed ops order (Mosquito incl. Event Spray, Tick, Ant, Fly Trap, Spotted Lanternfly, Yellow Jacket, Other). Replaced `summary.contractTypeBreakdown` with `summary.contractTypeGroups` (each family carries `count` + granular `members[]`); card retitled "Service type". Granular contract types still drive the rollup via `contractTypeGroupOf()`. Active Customers / Active Services definitions unchanged. rev 7 — regrouped the `/sales` service breakdown by granular **Contract Type** (`contract.agreement.name`) instead of the broad Service Type (`pest_contract.service_type.name`), which had an "Other" catch-all. Added `NormalizedContract.contractType`; renamed `summary.serviceTypeBreakdown` → `summary.contractTypeBreakdown` and the card to "Contract type" on `/sales` + `/tv/sales`. Active Customers / Active Services definitions unchanged. rev 6 — redefined the `/sales` headline metrics. Active Customers and Active Services are now tag-gated on a current-year tag (`CURRENT_YEAR` in categorize.ts, auto-advancing), not raw `status=Active` counts; added the service breakdown (`summary.contractTypeBreakdown`) rendered on `/sales` and `/tv/sales`; raw pre-gate counts preserved in `summary.debug.activeAllStatuses` / `activeServicesAllStatuses`. See §3.5 "Headline metrics". Buckets logic unchanged. rev 5 — synced doc with shipped reality per commit fe1f12d: marked PhoneBurner sync (cron, age-based folder routing, two-custom-field write) as LIVE rather than planned; replaced the speculative §6 file tree with the files that actually shipped; updated §4 webhook field-name bullets against the real Call End payload; moved webhook field-name reverse-engineering and several integration-quirk items from §9 OPEN to RESOLVED.)
**Project:** MS Analytics — Mosquito Shield of Long Island (Proganic LLC)
**Office ID:** 1512
...
| `/api/cron/mosquito-status` | `0 6 * * *` (06:00 daily) | Rebuild `mosquito_service_status` for `/service/overdue` — §5.5 (also aggregates `sprays_this_season` — §5.21) |
| `/api/cron/wellness-feed` | `0 7 * * *` (07:00 daily) | Wellness-queue feeder — §5.21. **LIVE since 2026-07-21** (ops go; initial fill ran the same day) |

(The table above lists the integration-relevant crons; `vercel.json` also runs the report crons — snapshot 05:00, service-counts 04:00, leads-followup 07:00, resprays 08:00, close-rate 09:00, referrals 10:00, tech-roster 09:00.)
...
| `66255089` | **Wellness — Queue 2026** | Wellness campaign (§5.21): active customers with 2+ sprays this season awaiting their wellness call. **EXEMPT — never policed.** |
| `66255090` | **Wellness — Called 2026** | Wellness campaign: one dial attempt of any kind lands the contact here for the rest of the season. **EXEMPT — never policed.** |
...
1. Select tracked `phoneburner_contacts` rows still in a policed folder **or the wellness Queue (`66255089`, rev 51 — queue contacts wait to be dialed, so fresh notes help the CSR; wellness Called is deliberately excluded)** whose `last_notes_refresh_at` is NULL or >24h old (oldest-first, capped at `NOTES_REFRESH_LIMIT`, default 40).
...
8. POST to Pocomos:
   ...
   - Lead: `/jwt/1512/lead/{lead_id}/note` — **⚠️ CONFIRMED DEAD (rev 54): 404 on every real lead** (9/9 in the backfill). Lead calls currently get NO Pocomos note; finding a working lead-note path is a §9 open item.
9. **Return 200 immediately**, run the note write in `waitUntil` background

**Customer-resolve chain (rev 54 — two landmines fixed):** (1) `find-customer-by-office` is a **WEB-SESSION endpoint, not JWT** — the JWT client receives a 200 login-redirect JSON (`{"type":"redirect"}`) whose empty `results` made every resolve silently fail; the route now calls it through `getSessionedHtml`. (2) That endpoint returns **EMPTY for cancelled customers no matter the params** (probed `active=0/1`, `inactive`, `status`) — and win-back dials are all cancelled customers — so on a miss the route falls back to the enriched `customers` Neon table matched by **normalized phone + case-insensitive last name** (the conversion sweep's identity rule). The backfill script uses the stronger form of the same bridge (email first, then phone+last-name, against the full live JWT customer list).
...
**Format strings (verbatim — these are the strings the dedup keys off, do not edit casually):**
...
PhoneBurner → Pocomos (one Pocomos `summary`, multi-line — **REDESIGNED 2026-07-21 per ops**, rev 52):
```
{Campaign} call — {disposition}
Email sent: {name}          ← only when a one-touch email went out with the disposition
{csr_name} · {duration}s
Notes: {text}               ← only when the CSR actually typed something
```
- **Campaign** from the dialed folder (`campaignForFolder`): wellness folders → `Wellness`, lead folders (Fresh/General/Competitor/Financial/Follow Up) → `Lead`, cancelled buckets → `Win-back`, unknown → `PhoneBurner`.
- **NO recording line** (2026-07-21 finding): every recording URL in the captured `api_calldone` payloads — short private form AND the 245-char `recording_url_public` — **404s even when fetched intact** (the public form redirects to S3 which has no object; PB populates the fields even for unconnected VM-drop calls). The long form additionally wraps and breaks in the Pocomos note display. Recordings live in PB call history when they exist.
- **No `Notes: (none)`** — the line only appears for a real CSR-typed note (`isAutoDispositionNote` still filters PB auto-text).
- **Email detection is PROVEN against live sends** (rev 53, corrects the rev-52 "unproven" caveat): a one-touch email is recorded as a **contact-notes entry in the SAME `api_calldone` payload** — `-- {ts} by {agent} -- Email sent: {subject}` — timestamped at the call (NOT in `events.last_event`/`call_notes`, which stay null/auto-only). `extractEmailSent` parses ALL note entries (`parseNoteEntries`) and applies a **timestamp guard**: only entries in the newest same-day cluster (within 3 min of the newest entry's header time) count, because stale "Email sent" entries from PREVIOUS calls live in the history forever — verified on a real re-dial payload that carries only the prior call's email entry (correctly rejected). Header times are compared to each other, never to `start_time`/`end_time` (PB stamps those in CT while note headers are ET). Verified 4/4 on real payloads: three sends detected with the exact subject ("Are we living up to your expectations?"), the re-dial correctly null.
- **⚠️ Entry-header format drift** (found rev 53): modern note entries insert `by {agent name}` before the closing `--`. The old `ENTRY_HEADER` regex didn't consume it, so entry parsing (CSR-note extraction + the `[Pocomos]` loop guard) silently failed on every modern payload — fixed; `Email sent:` entries and our own round-tripped campaign notes are also now excluded from the `Notes:` line.
- **Replay/duplicate gate** (rev 53, after a live double-write): the webhook hard-skips the note write when a `webhook_log` row with the same PB `call_id` already has `note_written = TRUE` (expression index `webhook_log_call_id_idx`). Replays and PB retries log a `duplicate call_id` row and write nothing — proven live (second replay of the same payload blocked). This is also the dedupe gate for any future historical-notes backfill.

**⚠️ LOOP-GUARD LANDMINE (found + fixed 2026-07-21, rev 52): Pocomos STRIPS the 📞 emoji and stores a literal `?`** — the live notes read `? PhoneBurner Call — …`. The old read-back test (`startsWith("📞 PhoneBurner Call —")`) therefore NEVER matched a real round-trip and the Pocomos→PB dedup was silently broken. The guard is now the emoji-free regex `PB_NOTE_GUARD` in `webhookProcessor.ts` (shared by `notes.ts::classifySource`, which feeds leadSync + notesRefresh): it matches the new first line (`Wellness|Lead|Win-back|PhoneBurner call —/–/-`) **and every stored legacy form** (`📞 …` as written, `? …` as Pocomos stores it, bare `PhoneBurner Call —`), case-insensitive. The em-dash DOES survive the Pocomos round-trip (verified); hyphen/en-dash are tolerated anyway. **Writer and guard live in the same file — change them only together.** The `[Pocomos]` prefix guard for the other direction is unchanged. Backfill note: the 294+ stored `webhook_log.raw_payload` rows replay through `parseWebhook` → the NEW format, so a historical-notes backfill needs no shim.
...
2. **Scrape** — **add-on** eligible customers (~21%) with no balance and not brand-new get the per-page `GET /customer/{id}/service-history` scrape (Surface C, READ-ONLY, never switches the selected contract) so the date is mosquito-contract-specific. If the rendered table's contract isn't mosquito, we **RL rev 25** resolve the mosquito last-service date from the completed-jobs cache (`respray_jobs`, mosquito-only) — the mosquito contract is already identified (they're eligible), so we compute overdue/current from the completed-jobs date rather than flagging. `needs_check` (`reason=no_mosquito_service_on_record`) now only remains when there's an eligible mosquito contract but **zero** completed mosquito service on record. Live effect: needs_check **7 → 0** (`respray_jobs` is refreshed at the top of `refreshMosquitoStatus`).
...
- **`EXEMPT_FOLDERS`** (NEVER touched): Active Customer `66233602`, **Wellness — Queue `66255089`, Wellness — Called `66255090`** (rev 51 — the wellness campaign IS the "future active-customer calling project" this rule anticipated; both folders hold active customers on purpose, and policing either would empty the queue within an hour). Exemption is **structural** — the sweep only reads `POLICED_FOLDERS`, so anything not policed is already ignored. **RULE: an active-customer CALLING project owns folders that hold active customers on purpose — add each such folder to `EXEMPT_FOLDERS` (and NEVER to `POLICED_FOLDERS`) so this sweep keeps leaving them alone.**
...
- **Rule 4 — the season buckets PARTITION the active roster (rev 19; WC carve-out rev 44).** `New + New–Season-Skipped + Returning(active) === Active Customers` (active customers carrying any `{CY}` tag). An active `{CY}`-tagged customer who was **not** a real CY-1 customer goes to **New** or **New–Season-Skipped**. Season-Skipped = *real prior spray history with us, but sat out last season*. **⚠️ WESTCHESTER IS YEAR ONE (rev 44, ops):** WC territory was bought from a prior operator, so a WC customer's pre-CY **tags** came from that operator and we hold **no spray history** for them — such a customer is genuinely **New**, not one who skipped. Rule: a customer with the **`0-Westchester` tag (or ZIP 105xx–108xx fallback, `isWestchester`)** whose only prior-year evidence is a tag (no pre-CY spray in our data) → **New**. WC customers with real prior sprays, and all LI customers, are unchanged. **Live move: New 149 → 199, Season-Skipped 89 → 39 (50 WC customers moved, per ops "~50"), Returning 928 unchanged, total 1,166 preserved.** Computed in `SeasonBuckets` from service evidence, **not** from `categorize.ts`; all three /sales tiles read the taxonomy. The Returning tile shows the full box (`returningTotal`), which also includes `churnedReturners`; the reconciliation line names them.
...
|---|---|---|---|---|
| `duplicate_records` | Duplicate customer records | **59** | Same human, 2+ Pocomos records (Pocomos spawns a NEW record on lead conversion). **RL rev 25:** grouped by email, then clustered by **name identity** (same last name + fuzzy first name — nickname/typo tolerant) so a shared email alone doesn't flag (fathers/daughters); records named "…duplicate…" are dropped as known shells. A group needs 2+ same-person records where ≥1 carries a year tag, sprays, or active status. Links the anchor **and every twin**. (Was 83 under email-only; −24 = 25 shell records over ~15 groups + 9 different-people mismatches.) | Merge the duplicates, keeping the record with the live contract. |
...
**⚠ READ-ONLY landmines right next to the data:** the task rows carry `Mark as Completed` → **`POST /todos/{id}/complete`**, and the page offers **`/message/todo/new`**. Both are MUTATIONS. The scraper only ever GETs `message-board` + `lead-information` and POSTs the established `/leads/data` read feed.

**CLASSIFICATION — RL rev 25 (tasks and notes are SEPARATE).** Ops model: a task stays *In Progress* while a lead is being worked (due date pushed forward per touch); a completed task = done reaching out; **notes** are the record of contact. So the classifier reads BOTH the task board AND the **Lead Notes** panel (see the note scrape below). This **replaces** the old 4 buckets (Overdue / No task / No open task / On track):

| Bucket | Rule | Live |
|---|---|---|
| **never_reached** | No task EVER (open or archived) AND no notes this year — for-sure missed, the worst bucket. | **43** |
| **loop_not_closed** | Reached (notes/activity) but NO task ever completed and none in progress — talked to, never tracked; create a closing task. | **49** |
| **closed_out** *(rev 26)* | A task was COMPLETED and none is in progress — done reaching out; the closing description is the outcome. Its own section shows the closing description ("what they closed with"), completion date, salesperson, Not-Interested reason where set, and lead link. | **185** |
| **working_overdue** | In-progress task whose due date is in the past (days overdue shown). Being worked, just behind. | **6** |
| **working_on_track** | In-progress task, due today or later. | **7** |

**rev 26 — closed-out split + collapsible UI.** The old `loop_not_closed` (234) was the CLOSED-OUT bucket from the spec buried inside it: **185 leads whose task was completed** (done reaching out) vs **49 genuinely reached-but-untracked**. Split on `archivedTaskCount > 0`. The completed task's plain-text date (history rows have no `data-order`, so `parseUsShortDate` reads the last cell) is the completion date, and its description is the closing outcome; `reason_name` from `/leads/data` fills the Not-Interested reason where set. **UI: each bucket is a collapsible section** (the anomalies-card pattern — `CollapsibleSection`, header shows label + count, click to open; never_reached + loop_not_closed open by default), replacing the multi-select stat-box filter.

**LEAD NOTES scrape (added rev 25).** Notes are on `/lead/{id}/lead-information`, `#notes-table` (server-rendered, one row per note; the Date cell carries `data-order="YYYY-MM-DD HH:MM:SS"`). **The bulk `/all-notes-data` report is CUSTOMER-only** — it returns 597 customer notes for 2026, zero in the lead-id (5.9M) range, so lead notes must be scraped per-lead. `parseLeadNotes()` returns the year's note dates; count + latest feed the never-reached/loop-not-closed split. **Cost (measured): 290 board GETs + 290 note-page GETs = 580 requests in ~175s** at concurrency 5, 0 failures (the old `/message/todo/{id}/show` comment-timestamp scrape was DROPPED — the new buckets don't need comment timestamps). Fits the 300s budget.

**Why the reclassification:** the old model treated a missing/overdue TASK as the problem, but the team completes tasks and logs contact in notes, so "overdue task" was nearly empty while 185 leads sat in a vague "no open task". The new model asks the ops question directly — *was this lead ever actually reached?* **43 leads were never reached** (no task, no notes) — the real for-sure-missed set — vs 234 that were reached but have no active task closing the loop.

**PHONEBURNER CROSS-REF (thin — be aware).** `webhook_log.pocomos_id` is **NULL on every row**, so it can't be joined directly; the working path is the bridge **`webhook_log.pb_contact_id → phoneburner_contacts.pb_contact_id → pocomos_id`** (`pocomos_type='lead'`). Only a handful of scope leads have any call event. The header counts how many **never-reached / loop-not-closed** leads have PB activity (someone dialled them even though no task tracks it) — **4 today**.

**Files:** `src/lib/leads/followup.ts` (domain + scrape + cache; `parseTasks` + `parseLeadNotes`), `src/components/followup-view.tsx` (view; stat boxes double as bucket filters, default never_reached + loop_not_closed), `src/app/leads/followup/page.tsx`, `src/app/api/leads/followup/route.ts` (GET read / POST refresh, lock-guarded), `src/app/api/cron/leads-followup/route.ts` (cron `0 7 * * *`). New cols `leads_followup.notes_count` / `last_note_at`. Manual fill: `scripts/run-leads-followup.ts`.
...
**ATTRIBUTION RULES (ops-canonical, rev 24 — 2026-07-17; the rev-21 10-day window is GONE):**
- **CURRENT_YEAR only** — prior-year jobs are NEVER used.
- **A RESPRAY = ANY completed mosquito `Re-service` job this year.** No gap criteria — every re-service counts.
- **Attribution: the tech of the customer's MOST RECENT PRIOR completed mosquito job this year — prior jobs INCLUDE both `Regular`/`Initial` sprays AND prior `Re-service`s (the CHAIN rule).** So Nick sprays → Nathaniel re-services → Daniel re-services again ⇒ Daniel's respray blames **Nathaniel** (the most recent toucher), not the original sprayer. `isMosquitoJob` is the prior-job pool; `chain = isReservice(prior)`.
- **No prior mosquito job this year → "unattributed"** (shown, nobody blamed).
- **Rate per tech = attributed resprays ÷ his total mosquito APPLICATIONS (Initial + Regular) YTD** — denominator UNCHANGED (re-services aren't in it; a tech who only re-services accrues blame but has a 0 denominator, shown as "—").
- Card text: *"Respray = any re-service this year, attributed to the most recent prior tech on that customer — including prior re-services."*

**Weekly bucketing** stays keyed to the **BLAMED job's week** (the prior job carried on the attribution), so a Monday re-service of a Friday job lands in the Friday week and a weekly rate can't exceed 100%.

**Why the change:** ops decided every re-service is a redo worth attributing, and that the person who *last touched* the account owns the next redo (chain). The old 10-day window silently dropped the 11+ day re-services.

**LIVE NUMBERS (2026 YTD, 2026-07-17, rev 24):** 5,492 mosquito jobs stored. **99 re-service jobs · 99 attributed · 3 chain · 0 unattributed · 5,393 applications · team rate 1.84%.**

**Old (≤10d window) vs new (any re-service + chain), same current data — the 28 previously-excluded 11+ day jobs now count (+chain):**

| Technician | Apps | OLD | NEW | Δ | OLD % | NEW % |
|---|---|---|---|---|---|---|
| **Nicholas Rosales** | 1,059 | 27 | **33** | +6 | 2.55% | **3.12% — FLAGGED (1.70×)** |
| Nathaniel Tapscott | 1,049 | 17 | 25 | +8 | 1.62% | 2.38% |
| Jason McQueen | 668 | 8 | 11 | +3 | 1.20% | 1.65% |
| Lenin Nunez | 565 | 5 | 9 | +4 | 0.88% | 1.59% |
| Josef Matute | 545 | 4 | 7 | +3 | 0.73% | 1.28% |
| Cesar Barrerra | 500 | 8 | 9 | +1 | 1.60% | 1.80% |
| Daniel Castelo | 509 | 1 | 3 | +2 | 0.20% | 0.59% |
| Mark Ware | 254 | 0 | 2 | +2 | 0.00% | 0.79% |
| Reggie Brown | 222 | 0 | 0 | +0 | 0.00% | 0.00% |
| Bruce Ivey | 21 | 0 | 0 | +0 | 0.00% | 0.00% |
| **TEAM** | 5,393 | **70** | **99** | **+29** | **1.30%** | **1.84%** |

Invariants (verified): per-tech resprays sum === attributed (99), per-tech apps sum === applications (5,393), attributed + unattributed === re-service jobs. **Flagging** = rate ≥ `FLAG_RATE_MULTIPLE` (1.5×) team avg **AND** `FLAG_MIN_APPLICATIONS` (30)+ apps — only **Nicholas Rosales** flagged.

**Weekly leaderboard (rev 24, §on the page).** "This week on the board" — current week + last full week, per-tech sprays/resprays/rate, with **Best** (lowest rate) and **Watch** (highest rate) callouts requiring `WEEKLY_CALLOUT_MIN_APPS` (20)+ apps that week, plus tasteful auto-stats (zero-respray streaks, most-improved week-over-week, a perfect-week shout-out) shown to the team. Computed in `buildWeeklyLeaderboard` from the per-tech weeks.

**Chain + repeat customers (rev 24).** Each drill-down respray row shows the **blamed prior job + its type** and a purple **"chain"** badge when that prior job was itself a re-service. A **"Repeat respray customers"** card lists accounts with **2+ resprays this year** (count + chain count + Pocomos profile link) — a product/access/account signal rather than a tech one. Live: **8 repeat customers, 3 chains** total.

**RESPRAY RULE, MATURITY + TWO CLOCKS (rev 39, ops-clarified — CORRECTS rev 38).**

**(1) The ~9-day window is a BOOKING rule, not a completion rule.** Rev 38 read it as a completion
window and excluded 39 re-services (39% of them!) as "anomalies". Ops clarified: the office already
applies the ≤9-10 day test **when booking**; rain and capacity then push the visit later. So **every
completed re-service with a prior spray is a legitimate respray**, attributed to that spray's tech,
no matter how long the visit took. `RESPRAY_WINDOW_DAYS` now drives only the informational
**long-gap marker** ("late visit") in drill-downs and the wording of the rule text — it never
excludes anything.

**Restored: attributed resprays 60 → 99, team rate 1.11% → 1.84%** (back to the rev-24 level), with
**39 long-gap visits** flagged for visibility only.

**(2) PROVEN CLEAN now needs three conditions** (`MATURITY_DAYS` = **10**):
(a) 10+ days elapsed, (b) no completed re-service blamed on it, **and (c) no re-service on the books
for that customer.** (c) is the rev-39 addition and it is not computable from job history: a booking
made on day 9 can be performed on day 15, so "10 quiet days" alone would credit a streak that is
already broken.

**Pending-re-service detection — probed, then scoped.** The `#scheduled-table` on
`/customer/{id}/scheduled-services` (the same GET the ASAP phase already makes) carries a **Type**
column. Live probe over 166 customers: Type = Regular ×2,163, **Re-service ×1**, Initial ×1,
Inspection ×1; Status = Pending / Re-scheduled. So a booked-but-unperformed re-service **is**
visible (found customer 1302338, 2026-07-21, Pending) — and it is **rare (~1 in 166)**, which is why
it must be read rather than inferred. New `hasPendingReservice()`.

**COST (measured, not estimated).** ~**385 ms per customer**, and pooling does NOT help — concurrency
5 measured the same per-customer cost as serial, so Pocomos is the bottleneck. Scoping matters:

| scope | customers | cost |
|---|---|---|
| any spray in last 14d | 1,030 | ~6.2 min |
| **most recent spray 8–21d old (chosen)** | **514** | **198 s ≈ 3.3 min** |

The 8–21 day window on the customer's **most recent** spray is the correct scope: a spray under 8
days old is immature anyway, and past 21 days no booking is still open. It runs as a **nightly cron
phase** (budget-capped like the ASAP phase, flags stored on
`mosquito_service_status.pending_reservice` / `pending_checked_at`), so the board reads a flag at
zero cost. Live result: **1 pending re-service across 514 customers.**

**(3) TWO CLOCKS** — unchanged in shape, but the matured clock moved back one week because maturity
went 9 → 10 days:

| clock | drives | on 2026-07-19 |
|---|---|---|
| **VOLUME** | Workhorse, Road Warrior, spray counts, table `sprays` | **Jul 12–17** |
| **MATURED** | Iron Wall, Perfect Week, Most Improved, rates, Clean Streak, table `rate` | **Jun 28–Jul 3** |

Every award tile prints its own range; both boards carry the footer *"Resprays are booked within ~9
days of a spray; visits may complete later. Attributed to that spray's tech."*

**EXCLUSION IS AWARDS-ONLY.** Cesar Barrerra (head tech) and the `Z-*` route placeholders never win
an award but count in every number: **Cesar 500 apps / 9 resprays / 1.80%**. The YTD ticker reads
**5,393 sprays · 1.84%**, not the old 4,892. Probed: `Z-ASAP 01` and `Z-RLW 01` are real completed
work, not routing artifacts.

**SEASON PACE.** YTD applications vs the same calendar date last season, same Initial+Regular
definition both sides: **5,393 vs 5,185 by 2025-07-19 — +4.0%.**

**⚠️ "TEAM BEST" vs the CLEAN STREAK TILE — they are allowed to disagree (rev 40).** The ticker
reports the **season maximum streak across every tech**; the Clean Streak tile reports an **award
seat**, and `assignAwards` can seat a credible runner-up so that every tech leaves with something.
Live today: ticker **Team best — Daniel Castelo, 109**, tile **Clean Streak — Jason McQueen, 108**.
Both correct, and the old label ("Longest streak") made them look like a contradiction.

**The fix was the LABEL, not the number.** Echoing the tile in the ticker was rejected: it would have
printed "longest streak 108" while a 109 existed — trading an ambiguity for a falsehood. The ticker
now says **"Team best"** on both boards (the narrow board previously had NO label at all, which was
worse) and computes over **all** techs including Cesar and the placeholders, since it is a numeric
season stat and exclusion is awards-only. Cesar's streak today is 86, so the max is unchanged — but
if he ever tops it, the line is now honest by construction.

**CADENCE HEALTH (rev 37) — "share of gaps beyond our 11–17 day window".** A stat card on
`/service/resprays` turning the rev-36 §5.17 finding into something ops can watch. Of every gap
between a customer's consecutive mosquito services, what share ran **strictly longer than 17 days**:

| season | beyond / gaps | share | source |
|---|---|---|---|
| 2024 | 1,070 / 11,705 | **9.1%** | RealGreen export |
...
⚠️ **`> 17`, not `>= 17`.** 17 days is the TOP of the target window and counts as on-time. An earlier
ad-hoc analysis reported a ">17 days" figure that was actually `>= 17` — **12.3% / 35.4% / 38.7%**.
Same shape, higher level. `CADENCE_WINDOW = { min: 11, max: 17 }` in `lib/service/cadence.ts` is the
single definition; don't mix the two numbers in a report.

Gaps are computed **per customer within one season**, so no short-id ↔ web-id conversion is involved
(each year is self-consistent in its own id space). The current season costs nothing — it is derived
from the job rows `getRespraysReport()` already holds in memory — and the two completed seasons are
two small `LAG` aggregates in Postgres rather than 25k rows pulled into the request.

**Files/infra:** `src/lib/service/cadence.ts` (cadence health), `src/lib/service/resprays.ts` (fetch + parse + attribute + weekly + cache), `src/components/resprays-view.tsx`, `src/app/service/resprays/page.tsx`, `src/app/api/service/resprays/route.ts` (GET read / POST refresh, lock-guarded), cron `/api/cron/resprays` `0 8 * * *`. Cache table **`respray_jobs`** (every mosquito job: invoice_no PK, customer_id, technician, job_type, service_type, completed_date) — attribution + weekly + repeat lists all computed **on read** (5.5k rows, instant). Manual fill: `scripts/run-resprays.ts`.
...
### 5.14 Nav dropdowns, `/finance`, and the public fleet-counts feed (rev 23, 2026-07-17)

**Nav taxonomy (rev 59).** Top bar order: **Customers · Leads · Service · Finance · Texting · Requests · TV**. `components/nav.tsx` — the `NavDropdown` (click-driven, closes on outside-click/Escape/navigation, works on touch; a tab highlights on its own page AND any child page via `isActive` = `pathname === href || startsWith(href + "/")`). **Customers** = a plain tab to `/sales` (renamed from "Sales", label only — every route unchanged; the old Sales dropdown's "Paused — open balance" child is gone, Finance owns `/finance`). Dropdowns: **Leads** (Close rate `/leads`, Follow-ups `/leads/followup`), **Service** (Overdue sprays, Respray performance, Route board `/service/board`), **TV** (Sales board, Tech board, Tech board — narrow, Route board `/tv/board` — every item new-tab, see the TV-dropdown block below). Plain tabs: Finance, Texting, Requests. `/combined` and `/calling` placeholders are DELETED — permanent redirects to `/sales` / `/leads` live in `next.config.mjs redirects()`; the real calling dashboard reclaims a nav spot when the PhoneBurner backfill ships. The `/service` registry landing page still exists by URL but is not a nav entry point. **STANDING RULE: every new page MUST be added to the nav in the same build that creates it — not in the nav = not shipped** (also in CLAUDE.md + the dashboard-conventions skill). Verify: `scripts/verify-nav-taxonomy.ts`.

**⚠️ LANDMINE — a route segment containing a DOT 404s on Vercel (rev 34).** The fleet-count CSV
endpoints were served from `app/api/fleet-counts.csv/route.ts` and
`app/api/fleet-counts/{customers,services}.csv/route.ts`. Those **compile, appear in the build
manifest, and return 200 + `text/csv` under `next start`** — and **404 on Vercel for every client**.
Vercel resolves extension-looking paths against the static filesystem first and never dispatches to
the function. This is what broke Google Sheets IMPORTDATA (`Resource at url not found`); it was
**not** the user agent, not middleware (its matcher is `/texting` + `/api/texting/*` only), and not
a redirect. Diagnosis that isolates it in one step: the same URL on a local production build vs
production.

**Fix:** the handlers now live at **dot-free** paths — `/api/fleet-counts/{both,customers,services}`
— and the public `.csv` URLs reach them through `beforeFiles` **rewrites** in `next.config.mjs`.
Rewrites, not redirects, so a sheet's existing formula keeps working and never sees a 3xx
(requirement: 200 + `text/csv`, no hop). `beforeFiles` because that is precisely the phase that was
swallowing the paths. **Public URLs are unchanged** — `/api/fleet-counts.csv` (header + both
values), `/api/fleet-counts/customers.csv` and `/api/fleet-counts/services.csv` (single bare
number). **Never add a route directory with a dot in its name.**

**TV dropdown (rev 32).** A fourth dropdown — **TV** (Sales board → `/tv/sales`, Tech board → `/tv/techs`, Tech board — narrow → `/tv/techs/tall`) — so the three Yodeck boards are reachable from the dashboard instead of by typed URL. Two deliberate departures from the other dropdowns:
- **Every item opens in a NEW TAB** (`target="_blank" rel="noopener noreferrer"`). `Shell` renders `/tv/*` **without the nav** (kiosk chrome), so an in-tab link strands the user on a board with only the back button out.
- **New optional `NavLink.matchPrefix`** (`"/tv"`), because the TV children span `/tv/*` while the tab's landing `href` is `/tv/sales` — plain `isActive(href)` would leave the tab dark on `/tv/techs`. It is opt-in precisely so **Sales does not** inherit it: Sales' `/finance` child has its own top-level tab, and matching on children would light two tabs at once. Verified: only "Finance" is active on `/finance`.

`scripts/verify-nav-tv.ts` asserts the whole contract in a real browser — closed before click, three items with the right hrefs/labels, `_blank` + `noopener` on each, Escape closes, and the no-double-highlight invariant. (Dropdown children don't exist in the DOM until the click, which is exactly why this can't be a curl check.)

**`/finance`.** `app/finance/page.tsx` reads `getOverdueReport()` and renders the **shared `PausedBalanceCard`**. That card + the `RowTable` it uses were extracted from `overdue-view.tsx` into **`components/service-rows.tsx`** (exports `RowTable`, `PausedBalanceCard`, `fmt`, `money`, `shortDate`, `POCOMOS_BASE`); `/service/overdue` imports the same component for its in-place paused section — **one implementation, two pages**. Finance is scaffolded for payment-retry review next.
...
### 5.14b `/finance` cash-register moment + Collections Mode (rev 55, 2026-07-22)

**What it is.** When staff collect a payment from a paused-balance customer (manually, in Pocomos — the hub NEVER touches payments), the customer's open balance drops to $0 and they leave the paused bucket. Rev 55 detects that transition and celebrates it: a cash-register chime, a green flying "+$412 — Jane D." per cleared customer, and a "Collected $X since you last looked (N customers)" line. **Display-only, browser-only — never `/tv/*`** (self-guarded on pathname like the feedback bubble).

**Detection source (probed 2026-07-22).** Option (a) — diff `mosquito_service_status.open_balance` against a fresh Unpaid-Invoices pull — over option (b) invoice-level diffing: a "clear" is balance-level by definition, and invoice-level state adds machinery without reliability. Probe findings: one pull costs **2.4–3.4s** (token GET + report POST, 63 customers / 83 invoices); **two back-to-back pulls agreed exactly** (no transient omissions observed); paused roster 10 rows, 0 would-clear at probe time. Guards, because a wrong celebration is worse than a missed one:
- **FULL clears only** (stored > 0 → fresh exactly 0). Partial payments update the stored balance, never ring.
- **Empty-report guard** — 0 parsed customers = a broken report shell, not 63 simultaneous payments; detection skips.
- **Eligibility guard (refresh path)** — the prior-balance snapshot is read BEFORE `pruneStale`, and only still-eligible customers can ring, so a cancelled/ineligible customer's row vanishing is not a payment. (Residual known gap: a voided/credited invoice also reads as a clear — rare, accepted.)

**Ring-once idempotency.** `balance_clearances` has a unique index on `(pocomos_id, (detected_at AT TIME ZONE 'UTC')::date)`; every writer inserts `ON CONFLICT DO NOTHING` and only a returned row rings. Poll + visibilitychange + nightly refresh can all see the same clear — exactly one logs it. The collections path additionally zeroes the stored row (`status='cleared_balance'`, a bucket no roster renders) so later observers diff against 0; the next full refresh recomputes the customer's real status.

**Pieces.**
- `balance_clearances` (id, pocomos_id, full_name, amount_cleared NUMERIC, detected_at, source `'refresh'|'collections'`) via `initSchema()`.
- `lib/finance/clearances.ts` — `logClearances` / `listClearancesSince` / `runCollectionsCheck` / `detectRefreshClearances`.
- `refreshMosquitoStatus` — BALANCE-CLEARANCE phase after the bulk upsert (both the 06:00 cron and "Refresh now" detect); failure never fails the refresh; `RefreshMeta.balanceClearances`.
- `GET /api/finance/clearances?since=ISO` — clearances newer than the caller's marker + `serverNow` (client clocks untrusted).
- `POST /api/finance/collections-check` — one fresh pull diffed against the paused roster; 10s `sync_state` soft-lock (`busy: true` → caller skips the tick); `maxDuration 60`.
- `components/finance-collections.tsx` (`FinancePausedSection`) — wraps the shared `PausedBalanceCard` (new optional `headerExtra` + `rowExtraClass` props; `/service/overdue` unaffected).

**Collections Mode (the near-real-time loop).** Staff workflow: sit on `/finance`, open a customer's Pocomos profile, run the card (~15–20s), tab back, next customer. "Start collections session" (button in the card header) → while ON: re-check on `visibilitychange` (tabbing back) + a 20s fallback poll; fresh clears ring per-clear, the row flashes green and slides out (`prefers-reduced-motion` → fade only), a session tally accrues ("This session: $X · N customers"); auto-stops after 10 quiet minutes (activity = start / tab-back / a clear); per-tab persistence in sessionStorage. **The start click is the user gesture that unlocks the AudioContext**, so autoplay never blocks mid-session.

**Passive path** (for whoever did NOT run the session, e.g. next morning): on `/finance` load, fetch clearances newer than the per-browser localStorage marker (`ms_clearance_seen`); first visit initializes the marker silently (celebrating a month of history is noise). One chime + up to 8 staggered flyers + the collected line, then the marker advances to `serverNow`. Autoplay blocked → visual runs anyway + a "Replay sound" speaker affordance. Mute toggle (lucide `Volume2`/`VolumeX`) persisted in `ms_register_muted`. Sound (rev 63): the big MECHANICAL cha-ching — `public/sounds/register.wav` (1.7s: C7 double bell + lever clunk + drawer slide + coin rattle + stop thock), synthesized by `scripts/make-register-sound.ts` (no licensed asset; deterministic; normalized hot at 0.95 — the retired in-browser WebAudio chime ran at ~0.2 gain and was too small). Swap the file to change the sound; the TV sale-bell WAVs are separate (`make-sounds.ts`).

**Unmeasured (flagged):** Pocomos's internal latency from card-processed → invoice-marked-paid in the Unpaid report. The first real collections session validates it; if it lags minutes, the visibilitychange check simply catches the clear a cycle later.

**`open_balance` means OVERDUE balance — installment-safe (rev 57 probe).** Ops correction: the paused list is customers with OVERDUE payments, not everyone who owes anything — installment customers may owe future installments after a successful collection. Probed per-invoice (2026-07-22, `scripts/probe-unpaid-duedates{,2}.ts`): the `/finance/unpaid-data` report returns **only invoices due as of today** — all 83 rows had due dates ≤ today (max exactly today; statuses only "Past due" 46 / "Due" 37), zero future-due rows, even though our request's date window extends through next year Dec 31 (the form's aging buckets are past-due ages, so a not-yet-due invoice has no bucket). A live installment row ("Installment 1 of 4 – 2025 Auto-Renew", due 04/01/25) confirms installment invoices surface only once due. Consequences: (1) `open_balance` is the **overdue** balance by construction; (2) the clearance zero-test is correct for installment customers — paying the overdue installment(s) rings the register and drops them from the roster even when future installments remain; (3) they legitimately re-enter the roster when the next installment comes due and goes unpaid, and a later clear that day can ring again only across a UTC-day boundary (the per-day dedupe). Each due-date row also carries an explicit `Status:` text and the Due date in `MM/DD/YY` — parseable if a past-due split is ever needed.

### 5.15 `/tv/techs` — "Tech Board", the shop-TV recognition screen (rev 28, 2026-07-19)

A Yodeck-ready, self-refreshing weekly awards board for the shop TV. **Yodeck webpage-widget URL:
`https://ms-operations-hub.vercel.app/tv/techs`** — 1920×1080 landscape, no interaction, no auth.

**REFERRAL TROPHY + MONTH-LONG BOOST (rev 41) — the deferred TV-TECHS-REFERRAL, now shipped.**

The trophy's data source is the **weekly PAYROLL Google Sheets**, not Pocomos — Pocomos has no
referral attribution (lead `marketing_type` is ~93% blank), which is why this sat in the backlog
since rev 28. Source: the **"Payroll Calculator - MS" PARENT folder**
(`1EP1fMZrPMaCnx3lY2rt3DYOM-v8kwAwF`, shared read-only with the service account) → a per-year
subfolder ("2026", "2027", …) **resolved by name at runtime** (rev 43 — see below); one sheet per
week ("MS Payroll Calculator - YYYY-MM-DD", Friday week-ending), one tab per tech.

**Detection rule (probed against 6 real weeks):** a referral is an OTHER PAY row of **exactly $50**
whose NOTES cell holds the referred customer's name. **BOTH halves are load-bearing** — week
2026-06-12 carries `TAPSCOTT — $10 — "Westchester walkthrough"`, which a notes-only rule would have
put on a TV as a customer named "Westchester walkthrough". ⚠️ **The sheet repeats a referral across
consecutive weeks** (Channa Noiman in 07-03 AND 07-10; Mina Becher in 06-19 AND 06-26), so
`referral_awards` is keyed on **(technician, customer)** and keeps the LATEST week-ending (earliest
would have expired Nathaniel's boost the day it was detected).

**Scan-back result: Nicholas Rosales → Channa Noiman (wk 2026-07-10), Nathaniel Tapscott → Mina
Becher (wk 2026-06-26).** Payroll tab names ("ROSALES, NICHOLAS") map to Pocomos spellings
("Nicholas Rosales") by 2-token match (`matchTechnician`) — survives "BARERRA, CESAR" vs "Cesar
Barrerra" and "MATUTE AYALA, JOSEF" vs "Josef Matute".

**Award behaviour.** (1) **Top billing** — a spinning gold trophy (`REFERRAL_AWARD`,
`topBilling+spin`, the hooks reserved at rev 28; `animate-spin-slow` = `spin 4s`) in the hero slot:
"Customer Referral!" + tech + **"referred {customer}"**. **NO dollar amount anywhere.** Multiple
referrers → multiple trophies (hero keyed `award.id:technician`, not `award.id`, or they collide).
(2) **Month-long boost** — `getBoostedTechs()` returns everyone inside a referral's celebration
month (week-ending + 1 calendar month); EVERY tile a boosted tech wins gets a gold star + border,
so the celebration outlasts the trophy week. (3) The trophy appears the week detected and holds for
the month.

**Ingestion.** `lib/service/payrollDrive.ts` reads the Drive + Sheets APIs with **read-only** scopes
(`drive.metadata.readonly`, `spreadsheets.readonly`) via a service-account JWT signed with
`node:crypto` — **no `googleapis` dependency**. Nightly cron `/api/cron/referrals` (`0 10 * * *`)
scans the newest 6 sheets.

**⚠️ YEAR-PROOF FOLDER RESOLUTION (rev 43).** The folder is NOT hardcoded. `resolveYearFolderId()`
queries the shared parent for a subfolder whose name **exactly equals `CURRENT_YEAR`**
(`'{parent}' in parents and name='2026' and mimeType=folder`) — the `= name` match skips the sibling
`0-Templates` folder, and `CURRENT_YEAR` is year-relative, so **January 2027 needs zero code or
config change**: someone drops a `2027` folder in the parent and the scanner finds it. Envs:
`PAYROLL_DRIVE_PARENT_FOLDER_ID` (defaults to the real parent `1EP1fMZrPMaCnx3lY2rt3DYOM-v8kwAwF`);
optional `PAYROLL_DRIVE_FOLDER_ID` pins a specific year folder and skips resolution (backfill/test);
optional `PAYROLL_DRIVE_YEAR` overrides the year. **If the year folder is missing, the scan throws a
clear "create a {YEAR} folder" error and the cron skips — it never crashes.** THE YEARLY RITUAL is
therefore just: **create a `{YEAR}` folder inside "Payroll Calculator - MS", nothing else.**

⚠️ **DORMANT until a service account is provisioned**: `hasDriveCredentials()` is false without
`GOOGLE_SERVICE_ACCOUNT_EMAIL` / `GOOGLE_PRIVATE_KEY`, so the cron **no-ops with a reason and returns
200** rather than failing. Until then `referral_awards` is the **manual/seed path**
(`scripts/seed-referrals.ts`), and the payroll sheets stay source of truth either way — the cron
takes over with zero code change once the env vars exist. Ops has already shared the **parent**
folder (not just 2026), so resolution works the moment credentials are set. ⚠️ `recordAwards`
(repeat-avoidance) records only the six weekly SEATS, never the trophies — two trophies share
`award_id="referral"` and would collide on `(week_start, award_id)`.

**⚠️ EMOJI-FREE** like the rest of the board (§5.16) — trophy and boost star are lucide SVG
(`Trophy`, `Star`). `scripts/verify-referrals.ts` asserts trophy + spin + boost star + customer name
+ **zero `$`** + zero emoji + zero clipping at 1080p and all four tall sizes.

**No new scrape and no new cron.** The board is computed on read from two caches existing nightly
crons already fill: `respray_jobs` (`/api/cron/resprays`, 08:00 — every mosquito job YTD with
technician/type/customer/date) and `mosquito_service_status` (`/api/cron/mosquito-status`, 06:00 —
carries `route_code`). Route join is `mosquito_service_status.pocomos_id = respray_jobs.customer_id`;
**probed 2026-07-19: 617/619 of the week's jobs join to a route (99.7%), 44 distinct routes — the id
spaces match, no short/web conversion needed.** Page sets `dynamic`/`fetchCache = "force-no-store"`/
`revalidate = 0` (mutable-cache rule); the client reloads itself every 10 min.

**The board week is the last COMPLETED Mon–Sun week, never the in-progress one.** Probed 2026-07-19:
the live week had 609 sprays and **0** resprays, because a respray is bucketed to the week of the
spray it is blamed on and re-services arrive days later. Running awards on the live week makes Iron
Wall / Most Improved / Perfect Week meaningless (everyone at 0.0%). The clean-streak walk is capped
at the board week's end for the same maturity reason — uncapped, it counted the immature live week
and handed 🎯 to the tech with the week's WORST rate (117-streak vs a real 138).

**Screen rules.** Cesar Barrerra (head tech) is excluded from every award and the table, along with
the `Z-ASAP 01` / `(unassigned)` Pocomos placeholders (`EXCLUDED_TECHS`). **No negative callouts** —
nothing is ranked last, Most Improved only ever shows a genuine rate *drop*, and the bottom table is
sprays + rate only. **Route-fair:** volume never judges quality — it gets its own positive award
(⚡ Workhorse) and is not mixed into any rate award.

**Awards** (`AWARDS` in `lib/service/tech-board.ts` — adding one = an entry + a `computeCandidates`
case; the view needs no change): 🎯 Clean Streak (min 20 in a row) · 🛡️ Iron Wall (rate, min 20
sprays, floored at the team week rate) · ⚡ Workhorse (properties, min 20) · 🗺️ Road Warrior (distinct
routes, min 3) · 📈 Most Improved (genuine rate drop, both weeks ≥20) · 🏅 Perfect Week (0 resprays,
20+ sprays). `AwardDef.topBilling`/`.spin` are the hero-slot + spinning-trophy hooks reserved for the
DEFERRED referral trophy (BACKLOG **TV-TECHS-REFERRAL**); no award sets them today.

**Every tech gets an award — via maximum bipartite matching (Kuhn's), not a greedy hand-out.**
Greedy fails here: one strong tech sweeps three awards, and "covering" the leftover tech demotes a
real superlative into a weak one (the first cut produced *"Clean Streak — 2 in a row"* over a
genuine 138-streak). Techs are matched scarcest-candidacy-first; leftover awards then go to their
top credible candidate; finally a tech repeating last week's award yields it if he's covered
elsewhere. Last week's assignment lives in **`tv_tech_awards`** (`week_start, award_id, technician`),
written when the board is computed — tiny and purely cosmetic; losing it just relaxes the repeat rule.
Candidate lists are **floored** (`MIN_CLEAN_STREAK` 20, `MIN_ROUTES` 3, Iron Wall ≤ team rate) so a
matched non-top holder still shows a real achievement, and **blurbs are descriptive, never
superlative** ("Properties serviced this week", not "Most properties") — the matching can seat a
non-top candidate, so a "Most …" blurb would be a falsifiable claim printed next to a number that
contradicts it. A tech with no credible candidacy is simply off the grid (still in the table);
inventing a superlative for him would be worse than silence.

**Live 2026-07-19 (board week Jul 6–12, all 5 non-Cesar techs covered):** 🎯 Josef Matute 38 in a row ·
🛡️ Jason McQueen 1.1% · ⚡ Nathaniel Tapscott 87 properties · 🗺️ Nicholas Rosales 13 routes ·
📈 Daniel Castelo 1.0%→0.0% · 🏅 Daniel Castelo 75 sprays 0 resprays. YTD ticker: 4,892 sprays ·
1.84% team rate · longest clean streak Daniel Castelo 138.

### 5.16 `/tv/techs/tall` — narrow tech board + weather strip (rev 29, 2026-07-19)

The companion to §5.15 for the **right-hand column** of the Yodeck layout. **Widget URL:
`https://ms-operations-hub.vercel.app/tv/techs/tall`**. Same board data (`getTechBoard()`), same
rules (no negatives, Cesar excluded, last-completed week) — **no new scrape, no new cron**. The
landscape `/tv/techs` is unchanged.

**Layout, top to bottom:** weather strip · "Tech Board" + week · the six awards as **tiles** (the
landscape board's visual language, compacted: colored uppercase award LABEL + emoji on top, first
name big, stat line) · YTD ticker. **No bottom table on this variant.**

**ALL SIX AWARDS ARE ALWAYS VISIBLE — the rotation is GONE (rev 31).** Rev 30 rotated 3 tiles at a
time on the short slot, on the theory that six tiles at ~44px each were unreadable. That was solving
the wrong problem: the tiles were small because the grid was **one column**, wasting the slot's full
width. A 2×3 wall on the same ~470×430 slot gives **222×89px tiles** — more than double rev 30's row
height, with all six on screen at once and no slideshow to wait through. A shop TV is glanced at,
not watched; anything that makes a viewer wait 15s for the other half of the board is a regression.

**The grid reshapes instead** (`columnsFor(w, h)` — explicit rules, not aspect-ratio optimisation,
which quietly disagreed with what each real slot needs):
- **tall AND narrow** (`h > w && w < NARROW_MAX` 700) → **1 column** (600×900 → 1×6, tile 571×110).
- **wide** (`w ≥ WIDE_MIN` 1000) → **3 columns** (1200×600 → 3×2, tile 386×191).
- **everything else** → **2 columns** — including the real Yodeck slot (470×430 → **2×3**, tile
  222×89) and 550×700 (2×3, tile 259×165).

**Tile type is sized in `em` off the MEASURED TILE BOX, not viewport units** — the parent computes
`basePx = min(tileH × 0.20, tileW × 0.125)` (clamped 8–34px) and the tile's internals are all `em`.
**Height sets the scale; width caps it** — height alone let a long stat ("75 sprays, 0 resprays")
truncate on a squat 2-up tile. Viewport-relative type can't know how big its cell is: at 550×700 it
overflowed and clipped names mid-glyph while the DOCUMENT fit perfectly, so the document-level
overflow check sailed straight past it. `scripts/verify-tv-techs-tall.ts` asserts **per-tile**
`scrollHeight ≤ clientHeight` via a `data-award-tile` hook, **plus the measured column count**, at
every size — six labels present at all four sizes, 0 clipped.

Measuring the region is feedback-loop-safe: the awards wall is the only `flex-1` element, so its
height comes from the `shrink-0` chrome around it, never from its own content.

**Responsive by design — no breakpoints.** The Yodeck webpage widget IS the viewport, so chrome
sizes are `clamp(min, Nvmin, max)` rather than fixed px or Tailwind steps; Yodeck's zoom just changes
the viewport and the type follows. `vmin` (not `vw`) because the screen must also survive a WIDE
SHORT slot (1200×600), where `vw` blows the type past the available height. Verified at 470×430,
550×700, 600×900 and 1200×600 with zero document overflow AND zero per-tile clipping (a TV widget
cannot scroll). **Priority when short:** weather, header and ticker are `shrink-0` so they always
render; the awards block is the single `flex-1 min-h-0` region, so a short container compresses the
awards instead of pushing anything off-screen.

**THE SPRAY WEEK IS SUNDAY–FRIDAY (rev 36, ops).** The crew sometimes sprays Sunday and **never**
sprays Saturday. Every week boundary in the service domain now derives from one function —
`resprays.ts::weekStart()`, which returns the **ISO Sunday** of a date's week — so the tech board
(landscape + tall), the `/service/resprays` weekly leaderboard, the Most Improved week-over-week
comparison and all week bucketing moved together.

- **The bucket is Sun–Sat (7 days) with a structurally empty Saturday**, kept deliberately: bucketing
  Sun–Fri only would let a stray Saturday job (an exception, a mis-keyed date) fall through the gaps
  and vanish from every weekly total. **Displays say Sun–Fri** (`TechBoard.weekEnd = weekStart + 5`)
  so the board never advertises a day nobody worked.
- **ROLLOVER RULE — the board flips on SATURDAY.** `boardWeekStart()` = the most recent Sun-start
  week whose Friday has fully ended. Friday Jul 17 → board Jul 5; **Saturday Jul 18 → board Jul 12**;
  Sunday Jul 19 → Jul 12; holds through Friday Jul 24; Saturday Jul 25 → Jul 19. Saturday, not
  Sunday, is the clean choice: nothing can land in a Sun–Fri week after its Friday, so waiting a
  further day would only show a staler week for no gain.
- ⚠️ **A different week convention exists elsewhere and was left alone**: `categorize.ts::
  startOfSaturdayWeek` is a **Sat–Fri** week belonging to the SALES snapshot convention. Different
  domain — do not "unify" them.
- ⚠️ **`tv_tech_awards` rows written before rev 36 are keyed to Mondays.** The repeat-avoidance
  lookup (`readLastWeekAwards`) keys on the prior week's Sunday, so it finds nothing for the one
  transition week and simply doesn't suppress a repeat award that once. Self-heals after one week;
  no backfill worth doing.

**WEEK TIE-OUT (rev 36).** `scripts/verify-week-tieout.ts [tech] [weekStartISO]` proves the board's
counted sprays equal the raw Pocomos completed-jobs rows for the same span, broken down by service
type × job type. It fetches the **live report**, NOT `respray_jobs` — the cache stores only
mosquito-family jobs, so reconciling against it would compare the cache to itself and could never
surface an Add-On Tick row. Live result for Nathaniel Tapscott, week of Jul 12–17 2026:

| service type · job type | counted | excluded | why |
|---|---|---|---|
| Mosquito Control · Regular | 93 | — | |
| Natural Mosquito Control · Regular | 19 | — | |
| Mosquito Control · Initial | 3 | — | |
| Add-On Tick Control · Regular | — | 12 | not a mosquito service type |
| Other · Regular | — | 9 | not a mosquito service type |
| Natural Add-On Tick Control · Regular | — | 7 | not a mosquito service type |
| Mosquito Control · Re-service | — | 2 | job type is not an application |
| Other · Initial | — | 1 | not a mosquito service type |

**146 Pocomos rows = 115 counted + 31 excluded**, board's own `weekStart()` count = 115 ✓, and the
117 mosquito-family rows (115 applications + 2 re-services) match the `respray_jobs` cache exactly.
Sunday and Saturday both had **0** jobs, confirming the Sun–Fri working shape.

**⚠️ NO EMOJI ON THE SHOP TVs — EVER (rev 35).** Yodeck's Linux browser ships **no color-emoji
font**, so 🎯🛡️⚡🗺️📈🏅, the weather glyphs and 💧 rendered as **empty boxes (tofu)** on the actual
screens while looking perfect in every desktop browser and every screenshot we'd taken. Emoji are a
font lookup; an inline SVG is geometry, so SVG renders identically everywhere.

All TV icons now come from **`components/tv-icons.tsx`** (lucide-react):
- `AwardIcon` — chosen by `award.id`, each in a gradient badge disc with a soft radial glow and its
  own accent: Clean Streak pink `#f472b6` · Iron Wall blue `#60a5fa` · Workhorse amber `#fbbf24` ·
  Road Warrior green `#4ade80` · Most Improved teal `#2dd4bf` · Perfect Week gold `#fcd34d`. An
  unknown id falls back to a slate medal, so a future award (the deferred referral trophy) is
  legible before it has a color.
- `WeatherIcon` — WMO code → lucide sun/cloud/fog/drizzle/rain/snow/storm, same bands the old emoji
  map used, tinted to the board's palette. Deliberately **unbadged**: four badge discs in the strip
  would fight the award wall below it.
- `PrecipIcon` — the old 💧.

**The emoji fields are DELETED from the data layer**, not merely unused: `AwardDef.emoji` is gone and
`ForecastDay.emoji` became `ForecastDay.code` (the raw WMO number). That's what makes "no emoji on
the TV" a property of the system rather than something to keep remembering — there is no emoji left
to render.

**⚠️ Icon colors are INLINE styles, not Tailwind classes.** A dynamic `text-${accent}-400` is purged
at build time and renders colorless in production — the same invisible-in-prod failure mode as the
emoji itself. Sizes are passed as a `size` prop in the caller's unit (`em` on the tall board so
icons scale with the measured tile type; `rem` on the landscape board).

**Verify:** `scripts/verify-tv-icons.ts` asserts **zero emoji codepoints in the rendered DOM** —
`innerText` *and* every `::before`/`::after` `content` (where a stray glyph could hide) — plus the
replacement SVG count, document overflow and per-tile clipping, at 1080p and all four tall sizes.
It writes screenshots to `scratch-tv-icons/` (gitignored): **look at them**, since "renders" and
"looks designed" are different checks and only one is automatable.

**⚠️ Verify `/tv/*` against a PRODUCTION build, not `next dev`.** During rev 30 the dev server served
CSS with core utilities (`h-screen`, `overflow-hidden`, `flex-1`, `shrink-0`, `grid-cols-4`) missing
from the stylesheet entirely — the classes were on the elements with no rules behind them, so the
shell sized to content and every overflow assertion failed against a page that was fine in
production. Two follow-up "fixes" were chased before the cause was found. Build, then
`npx next start -p 3100`, and **confirm the port is actually free first** (`netstat -ano | grep
LISTENING`, `taskkill //PID <pid> //F`) — a zombie server silently serves the OLD bundle and makes a
passing check meaningless.

---

### 5.17 Pre-Pocomos history (2021-2023) + the 5-season return-rate trend (rev 33, 2026-07-19)

The return rate had two points (24→25, 25→26) — a number, not a trend. Loading the three
pre-Pocomos RealGreen seasons extends it to **five consecutive pairs**, and the answer is a clear,
monotonic decline:

| pair | rate | returned / real | method |
|---|---|---|---|
| 2021→2022 | **82.3%** | 873 / 1,061 | spray-only (frozen) |
| 2022→2023 | **80.7%** | 962 / 1,192 | spray-only (frozen) |
| 2023→2024 | **79.6%** | 984 / 1,236 | spray-only (frozen) |
| 2024→2025 | **78.9%** | 1,007 / 1,276 | live (spray or tag) |
| 2025→2026 | **77.3%** | 948 / 1,227 | live, in progress |

**−5.0pp over five seasons, falling every single year.** Real-customer denominators over the same
span: 1,061 · 1,192 · 1,236 · 1,296 · 1,243 — the book grew while retention slid, so the decline is
a retention story, not a shrinking-sample artifact.

**Source.** `data/{2021,2022,2023} spray dates.csv` — the same 153-column RealGreen report as
`realgreen_jobs_2024.csv`, exported with different settings: **single `\r` line endings** (not
`\r\r`), `M/D/YYYY 0:00` 24-hour dates, and **a fully-blank comma-only row between every data row**.
`splitCsvLines` already tolerates the endings and `parseExportDate` ignores the time, but the blank
rows are commas (not whitespace) so they survive the splitter — `loadRealgreenHistoryYear` skips
them explicitly via `/[^,\s]/` and counts them as `blankRows`, because letting them fall through to
the `badId` guard would make a clean load report look ~50% broken. All three files parsed with
**0 badDate, 0 badId, 0 off-year jobs**; every job in them is mosquito-family (codes 12/12N/24/24N).

**Tables.** `realgreen_jobs_history` (year-keyed, one table for all three seasons — they arrive
together and are only ever read in aggregate) and `return_rate_history` (the three frozen pairs).
`realgreen_jobs_2024` is deliberately NOT folded in: it feeds the `/sales` anomalies card, whose
worklist must stay scoped to the years ops actually reconciles. Adding 2021-2023 short ids to that
card's "unmapped" union would bury a live 117-item worklist under long-churned customers nobody
will ever fix.

**⚠️ THE DESIGN DECISION THAT MATTERS — history is computed in RealGreen SHORT-ID space, never
through `customer_id_map`.** That map is built by matching export contacts against the CURRENT
Pocomos customer list, so it only resolves people who still exist in Pocomos. Measured on 23→24,
where both are computable:

- short-id space: **79.61%** (984/1,236)
- through the id map: **96.91%** (973/1,004)

The map **drops 232 of 1,236 denominator customers and inflates the rate by +17.3pp** — because the
customers it can't resolve are precisely the ones who churned, and a churned customer is exactly
what a return-rate denominator is made of. Both sides of every historical pair live in the same
RealGreen export universe and share the short id, so no id translation is needed at all. This is a
standing landmine: **never route a historical cohort through the id map.**

**COMPARABILITY SEAM (documented, quantified, and shown in the UI).** The three historical pairs are
**spray-only**; the live pairs also count "active with any {Y} tag" as returned ("signing up is
returning"), which has no analogue before Pocomos existed. On 24→25, the one pair computable both
ways: spray-only **76.93%** (997/1,296) vs the live rule's **78.92%** — a **≈1.9pp** seam. Small
enough that the five points read as one trend, big enough that it must be labeled. So the UI
**encodes the seam instead of hiding it**: the pre-Pocomos span of the sparkline is dashed with
hollow markers, those table rows carry a `spray-only` tag, and the footnote states the gap. A reader
must not mistake a change of method for a change in the business.

**Load path.** `scripts/load-history.ts` (one-off per season, idempotent — deletes and reloads only
its own year). It reads 2024's spray dates from the ALREADY-LOADED `realgreen_jobs_2024` table
rather than re-parsing its 13 MB CSV, so the anomalies card's table is never rewritten. It prints
both validations above every run, so the design decisions stay evidenced rather than asserted. Full
run: **7.9s** for all three files (32,698 jobs, 3,584 customer-seasons).
⚠️ `MOSQUITO_SERVICE_TYPES` is a **lowercased** normalized lookup set while the export landed
`agreement` in original case — compare with `LOWER(agreement)` or the match silently returns zero
rows (this bit once; validation 2 read 0.00% until fixed).

**Read path.** The frozen pairs are read from `return_rate_history` by `getHistoryPairs()` and
**prepended** to the live pairs in `computeReturnRatesAndBox()` — not recomputed per request. Those
seasons can never change again, and their job rows aren't in the pocomos-id space that function
operates in. Cost is a 3-row query.

**UI.** `/sales` gains a `ReturnRateTrend` sparkline above the existing table: one series, so no
legend (the card title names it) and only the endpoints direct-labeled, plus the endpoint delta
(`82.3% → 77.3% (−5.0pp)`). The y-axis is **not** zero-based — legitimate for a LINE in a ~75-85%
band, and the axis range is printed under the plot so the zoom is explicit. `/tv/sales` keeps its
two big live pairs (five would reflow that 2-up grid into three cramped rows on a room-distance
screen) and gets the arc as a one-line caption. The card's late-signup and tag-vs-spray sub-count
footnotes are scoped to the LIVE pairs — the frozen ones carry `lateSignupsTo = 0` /
`returnedByTag = 0` **by construction**, so listing them would assert a zero the history never
measured.

**Verify:** `scripts/verify-return-trend.ts` (5 pairs present, 5 sparkline points, the seam marker,
the trend header). ⚠️ It waits for the card to populate rather than using a fixed settle: a COLD
taxonomy takes **~80s** to build (it scrapes Pocomos), so a short settle reads the "Loading…"
skeleton and fails for the wrong reason. Warm the API first, or let the script wait.

**Weather** (`lib/weather.ts`) — **display only, nothing operational reads it**; it exists so the
Yodeck layout can drop its separate weather app. **Open-Meteo, free, NO API key.** Probed
2026-07-19 at lat 40.61 / lon -73.71 (Lawrence, Nassau County NY); the API snaps to its grid point
at 40.597/-73.702, same neighbourhood. Response is parallel arrays under `daily` (`time[]`,
`weather_code[]`, `temperature_2m_max[]`, `temperature_2m_min[]`,
`precipitation_probability_max[]`). Renders **today + the next 3** (4 columns) as day · WMO-code
emoji · high/low °F · precip %.

**Why the 30-min TTL is a module memo, not Next's fetch cache:** the page sets
`fetchCache = "force-no-store"` (it also reads the mutable Neon caches), which forces every fetch in
the segment to bypass Next's cache — so the TTL has to live in the lib. Best-effort per serverless
instance; a cold instance refetches, which is fine at this volume (one TV reloading every 10 min).
**Fail-soft:** any error returns a stale memo if present, else `null`, and the strip is simply
omitted — weather can never take down the tech board.

**Shared:** `components/use-auto-reload.ts` (`useAutoReload` + `TV_REFRESH_MS`) — both TV screens
self-reload on the same 10-minute cadence; extracted from `tv-techs-view.tsx` as a pure refactor.
...
CREATE TABLE mosquito_service_status (
  ...
  sprays_this_season INTEGER NOT NULL DEFAULT 0,  -- completed mosquito services this season, ANY type (rev 51, §5.21); aggregated from respray_jobs, no scrape
...
**`wellness_calls`** — the wellness campaign's one-call-per-customer-per-season guard (rev 51, §5.21). Inserted FIRST by the webhook fall-out, before the PB folder move, so the guard holds even when the move fails; re-fired webhooks no-op on the PK.
```sql
CREATE TABLE wellness_calls (
  pocomos_id  TEXT NOT NULL,          -- INTERNAL customer id (7-digit url_id)
  season      INTEGER NOT NULL,       -- CURRENT_YEAR at time of call
...
(rev 51 also adds **`respray_jobs.address`** — the completed-jobs report's street-only Address cell, kept so the wellness feeder pushes a street address to PhoneBurner with zero extra scrapes.)

**`balance_clearances`** — cash-register log (rev 55, §5.14b): one row per detected FULL balance clear (paused customer's open balance >0 → $0). Written by the mosquito refresh AND `/api/finance/collections-check`; read by `/finance`. Ring-once = the per-(customer, UTC-day) unique index + `ON CONFLICT DO NOTHING`.
```sql
CREATE TABLE balance_clearances (
  id BIGSERIAL PRIMARY KEY,
  pocomos_id TEXT NOT NULL,
  full_name TEXT,
  amount_cleared NUMERIC(10,2) NOT NULL,   -- the stored balance at detection time
  detected_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  source TEXT NOT NULL                     -- 'refresh' | 'collections'
);
CREATE UNIQUE INDEX balance_clearances_day_dedupe
...
### 11.2 Tooling (rev 27 — BUILD-SPEEDUP)

**Skills** (`.claude/skills/`, load with the Skill tool): `pocomos-scraping` (Pocomos surfaces,
DataTables 1.9, id conversion, landmines) · `dashboard-conventions` (cron+cache report pattern,
shared components, UI idioms). **`/ship <spec>`** (`.claude/commands/ship.md`) runs the full ritual.

**Script lib** (`scripts/lib/`, reuse — don't re-implement):
- `livecheck.ts` — Playwright live-page check. `checkLivePage(url, {clickText, expectText, absentText})`
  drives a real browser (clicks dropdowns/collapsibles, reads the rendered DOM). CLI:
  `node node_modules/tsx/dist/cli.mjs scripts/lib/livecheck.ts <url> [clickText] [expect...]`.
- `pocomos.ts` — `legacyDataTablesBody(columns, {start,length,sortColumn,sortDir,sort})` +
  `LEADS_DATA_COLUMNS` + session re-exports.
- `csv.ts` — `writeCsv(path, headers, rows)` / `readCsv(path)` (handles the `\r\r` export quirk).
- `neon.ts` — `sql` / `initSchema` / `getSyncState` / `setSyncState` re-exports.

**`scripts/verify-live.ts`** — post-deploy Playwright smoke test of the key pages + a Neon ping.
Run it (or `livecheck.ts`) after every deploy — **verify live via a real browser, NOT curl+regex**
(React comment nodes, en-dashes, attribute order, and click-to-render children all break regex).
...
### 5.18 In-dashboard feedback + `/requests` review queue (rev 42, 2026-07-19; upgrades rev 49, rev 56)

Staff-facing feedback capture with a review queue and a prompt builder that turns selected items
into a paste-ready `/ship` prompt. **No Pocomos, no cron** — feedback is user-submitted, not scraped.

**Rev-56 upgrade — "Record screen" (screen + mic narration).** Third capture option in the bubble;
"Attach file" and "Take screenshot" are UNTOUCHED (html2canvas + the markup step stay exactly as
they were — the screen-share picker is inherent to *recording* and accepted there, not migrated to
the screenshot path). Mechanics:
- `getDisplayMedia({video})` + `getUserMedia({audio: {echoCancellation, noiseSuppression}})` merged
  into one `MediaRecorder` stream. **The mic is the point** (staff talk through the issue) but a
  denied/failed mic only downgrades to a silent recording with a visible "recording without audio"
  note — never aborts. Mic audio comes from getUserMedia, NOT getDisplayMedia audio (tab/system
  audio is inconsistent cross-browser and isn't wanted).
- **Feature-detected in an effect**: no `getDisplayMedia` or `MediaRecorder` (mobile Safari etc.) →
  the button never renders.
- mimeType via `MediaRecorder.isTypeSupported`: vp9,opus → vp8,opus → plain webm → mp4 (Safari).
  **⚠️ Landmine (found live): `recorder.mimeType` comes back with `;codecs=vp9,opus` — the COMMA
  inside the parameter breaks data-URI parsing** (`data:video/webm;codecs=vp9,opus;base64,…` has an
  ambiguous comma). The client strips parameters when building the Blob (bare `video/webm`); the
  server validator + serving route tolerate comma-free params as a belt.
- **Max 60s** with a visible countdown, a Stop button, and a red pulsing mic-hot indicator
  (bottom-center pill); the feedback panel hides while recording (same trick as the screenshot).
  The user ending the share from the browser chrome lands on the same clean stop path. **ALL tracks
  (screen + mic) are stopped** on stop/discard/cancel/unmount — no orphaned live mic.
- **Size cap:** `MAX_VIDEO_BYTES = 3 MB` (next to `MAX_IMAGE_BYTES`). The ceiling is the ~4.5 MB
  Vercel request-body limit (base64 ×1.37 → ~4.1 MB on the wire), NOT the 60s. Bitrates
  `videoBitsPerSecond 350k` + `audioBitsPerSecond 32k` (opus ≈ 240 KB of the 60s budget); probed
  with a real Chromium MediaRecorder: **~1.24 MB raw per 60s** on the fake source, worst case
  ≈2.9 MB at the full bitrate. 1s chunks accumulate; **auto-stop at 95% of the cap** with a
  "size limit reached" note. Quality at these rates is fine for screen content — no chunked upload
  needed.
- After stop: **preview modal WITH audio** (`<video controls>` on the data URI) so the narration can
  be confirmed, then **Attach recording / Discard & re-record / Cancel**. Attached video shows as a
  compact playable element in the panel with a Remove link.

Storage/API mirror the image pattern: nullable **`feedback.video_data_uri`** (validated
`data:video/...;base64,` within cap, else silently dropped; a submission may carry image, video,
both, or neither), list rows carry **`hasVideo`** (never the blob), and **`GET
/api/feedback/{id}/video`** serves the real binary with the STORED content-type. `/requests` rows
with video render a **Video tile** (lucide icon) → the existing full-screen viewer with
`<video controls autoPlay>` — the click is the user gesture, so audio plays. (Streamed-webm quirk:
`duration` may read `Infinity` until buffered — cosmetic.) Rate limit unchanged. E2E verify:
`scripts/verify-feedback-video.ts` (real MediaRecorder via Chromium fake-source flags).

**Rev-49 upgrades.** (1) **Two attach options** — "Attach file" (existing) and **"Take screenshot"**,
which captures the current page with **html2canvas** (chosen over `getDisplayMedia`: no screen-share
permission prompt, no tab-picking — the reliable UX for staff; the feedback panel is hidden during
capture). The screenshot then opens a **markup step** (`feedback-markup.tsx`): a canvas overlay to
draw **arrows / boxes / freehand** in red (default; 6 colours), with undo/clear, before attaching —
composited screenshot + annotations, downscaled to the 2 MB cap. (2) **Name is REQUIRED** and
remembered in `localStorage` (`ms_feedback_name`) so each person types it once. (3) `/requests` gains
a **"By submitter" filter with per-person counts** (a pill row that AND-combines with the status
filter).

**The bubble.** `components/feedback-bubble.tsx`, a floating "Feedback" button bottom-right on every
dashboard page. Mounted only by the **non-TV branch of `Shell`**, and it self-guards on
`pathname.startsWith("/tv")` too, so it can never appear on a kiosk screen. The compact form takes
required text, optional name, optional image; it **auto-captures `window.location.href` + timestamp**
(server-stamped `created_at`). Images are **downscaled in the browser** (fit 1600px, JPEG q0.82)
before upload, so a phone screenshot lands well under the cap without the server seeing the original.

**Storage.** New `feedback` table (`id, body, submitter, source_url, image_data_uri, status,
created_at, updated_at`). **Images are base64 data URIs stored INLINE** — the cheapest path at this
volume (a few rows/week); no blob store to provision. Caps: body 4000 chars, image ~2 MB
(`MAX_IMAGE_BYTES`), and `image_data_uri` is validated to `data:image/...;base64,` or dropped. The
list query returns a `hasImage` flag, **not** the blob; the image is served on demand as a real
binary from `GET /api/feedback/{id}/image` so the list payload stays light.

**API.** `POST /api/feedback` (submit — no auth, **lightly rate-limited** ~8/min per IP in memory,
just to smooth accidental double-submits) · `GET /api/feedback[?status=]` (list) ·
`PATCH /api/feedback/{id}` (set status) · `GET /api/feedback/{id}/image` (binary) ·
`POST /api/feedback/build-prompt` (ids → prompt, and marks still-`new` items `selected`).

**`/requests` page** (nav tab **Requests**, top level). Newest first; each row shows text, source
page, submitter, date, `#id`, and an image thumbnail (click → full-screen). **Status cycles on
click** New → Selected → Shipped → Declined → New (optimistic, reverts on API failure). Filter pills
by status with counts; **Shipped stays visible** under its filter for review-what-we-did.

**Prompt builder.** Checkbox items + **Build prompt** → `buildFeedbackPrompt` (pure, in
`lib/feedback-prompt.ts`) emits a numbered list — each item's text, source page, submitter, date,
`#id` — wrapped in this repo's shipping-ritual header/footer (probe-first, docs rev bump,
commit/push/verify-live, per-item report). Copy-to-clipboard. Including an item **auto-marks it
Selected** (server-side, `markSelected` only promotes `new` items so it never demotes a
Shipped/Declined one).

`/requests` reads the mutable table → `force-dynamic` + `fetchCache = "force-no-store"` +
`revalidate = 0` (the /leads/followup stale-zeros lesson). Verify: `scripts/verify-feedback.ts`
submits with an image, confirms it on `/requests` with a thumbnail, cycles status, builds a prompt,
and asserts the bubble is present off-`/tv` and absent on `/tv/*`.

---

### 5.19 `/tv/board` + `/service/board` — digital route schedule board (rev 45; v2 rev 50; weekly-mirror rev 62)

**Weekly-mirror fixes (rev 62).** The grid is the **CURRENT Sun–Fri service week — 6 columns,
always** (was: today + next 4 workdays). Week = `resprays.ts::weekStart` (Sunday), bumped a week on
Saturday (the tech-board flip convention; NOT the Sat–Fri sales week). Sunday renders even when
empty; today gets a sky ring + TODAY badge; the header shows "Week of {start} – {end}".
`/tv/board` is strictly display-only (0 form elements, verified); `/service/board` is the admin
mirror and takes **`?week=YYYY-MM-DD`** to review any week ("(review)" tag). Sheet-empty days keep
the SAME tech-row format with name "—" ("not on sheet yet · Pocomos (mosquito only)" — those stop
counts are eligible-mosquito next-services and undercount the sheet's ~30/route by design; capped
at 9 rows + "+N more"). Weekly/special daycodes (WF1/WF2/WG1/RLW/TKO/ASAP/TMP/ANT…) render as
violet pills vs mono-sky numerics, with a legend chip; names/towns wrap instead of truncating;
sheet content mirrors verbatim (typos included). **URGENT ANNOUNCEMENT** banner (red, both boards)
from `board_announcements.urgent`; THIS/NEXT WEEK seeded from the physical board
(`scripts/seed-board-announcements.ts`, fills empty fields only). ⚠️ Parser: the label row says
both "DayCode" and "Day Code" — matched with `/^day\s*code$/i` (the strict match swapped
daycode/van on Jun 17–19). Real ANT sheet days: 5/18 Mark "303, ANT" · 5/19 Nathaniel "602, ANT" ·
6/15 Cesar "ANT" · 6/16 Nathaniel "608, ANT" (marker verified on 6/15–16 via the review override).

**v2 (rev 50): TECH-FIRST, sheet-primary.** The Master Routing sheet is now the PRIMARY source and
Pocomos secondary. Per day, one row **per tech** — NAME · DayCode · Van/Loc # · Towns · # Stops —
parsed from the CALENDAR (`masterRouting.ts::parseCalendar`, label-anchored columns, `Tech1..6`
placeholders dropped, name variants like Nick/Joseph kept as written). Days the sheet hasn't filled
fall back to the Pocomos daycode list. **Markers:** an **ant** bug icon on ANT daycodes + a
"**Ant needs 3 dry days**" caution when any of service-day/+1/+2 forecasts ≥55% precip; a **bolt** on
routes carrying a `NT - Electric Blower Only` customer (joined via Pocomos route_code = daycode);
a **legend** strip (LC/LI/BK/GN/QU/WC). **Right rail:** an **Announcements** panel (THIS/NEXT week,
`board_announcements` Neon table, edited from `/service/board`), a static **New customers — 1st
spray** box, and a **Shout-outs** panel (§5.20). Day-level **Notes:** line under each column, from
the sheet. **`/service/board`** (nav: Service → Route board) is the browser mirror — same
`TvBoardView` dark board + the announcement editor, shout-out form, and shout-out management. All
external reads (sheet, weather) 6–8s timeout-guarded so nothing stalls the TV. Yodeck-safe: SVG only,
inline styles, 1080p, no emoji, no clip.

### 5.20 Compliments / shout-outs (rev 50, 2026-07-20)

Peer recognition on the route board. **Roster** (`tech_roster`, `techRoster.ts`) is read from
**COLUMN A ONLY** of the "Technician & Asset Information" sheet (`TECHNICIAN INFORMATION` tab) — names
only; ⚠️ the sheet holds credentials/PII in other columns and the range is `A1:A` and never widens. A
real entry is `"Last, First"` (has a comma) → displayed "First Last"; section/asset rows ("LOCKER",
"OFFICE SPARE") have no comma and are excluded → **8 techs incl. head techs Cesar Barrera + Emanuel
McAuliffe** (compliment-eligible even though award-excluded on /tv/techs). Nightly cron
`/api/cron/tech-roster`. **Shout-out form** on `/service/board`: tech dropdown (roster), body
(≤160 chars), from-name (required, localStorage-remembered), optional customer → `compliments` table.
**Shout-outs panel** on both boards: visible **7 days**, newest first, rotates every 12s on overflow
(TV), SVG megaphone, no emoji. **Soft-delete** (hide/restore) from `/service/board`; hidden rows never
show. APIs: `POST /api/compliments`, `PATCH /api/compliments/{id}`.

### 5.21 Wellness-calls campaign — self-refilling PhoneBurner queue (rev 51, 2026-07-21)

A season-long calling campaign: every **active customer with 2+ completed mosquito sprays this season** gets exactly **one** wellness call. One dial attempt of ANY disposition (connected, VM, No Answer) removes them from the queue for the rest of the season; new qualifiers flow in automatically as they hit their 2nd spray. The spec's numbering called this §5.8, which was taken — it lives here as §5.21.

**Folders (created manually in the PB UI, probed by `scripts/probe-wellness-folders.ts` via `GET /folders`):** `Wellness — Queue 2026` = **66255089**, `Wellness — Called 2026` = **66255090**. Both are in `EXEMPT_FOLDERS` (`folders.ts` exports `WELLNESS_QUEUE_FOLDER` / `WELLNESS_CALLED_FOLDER`); **NEVER add either to `POLICED_FOLDERS`** — they hold active customers on purpose and the hourly sweep (§5.5b) would empty the queue within an hour.

**Spray counter — `mosquito_service_status.sprays_this_season`.** The spec assumed no bulk per-season count source existed and called for ~1,100 per-customer service-history scrapes. **It didn't need them:** `respray_jobs` (§5.12) is reloaded daily from ONE completed-jobs-report POST covering `CURRENT_YEAR`, holds every completed mosquito-family job keyed by internal customer id, and is **contract-independent** — it also covers the ~21% add-on customers whose default rendered service-history table isn't mosquito (exactly the population the scrape can't read). `updateSprayCounts()` (`refresh.ts`, exported; runs inside the 06:00 `refreshMosquitoStatus` after the completed-jobs phase) aggregates `COUNT(*)` per customer — any job type, matching `COUNT_ANY_SERVICE_TYPE` semantics. Zeroing of customers absent from `respray_jobs` happens ONLY when that run's completed-jobs refresh succeeded (year rollover legitimately empties the table in January; a failed refresh must never zero anybody). **Validated 2026-07-21: 5/5 random spot-checks matched an independent service-history scrape exactly** (`scripts/run-wellness-counts.ts`). Live distribution: 1,137 of 1,152 eligible at 2+ (mode = 5 sprays × 761 customers).

**`wellness_calls` table — the season re-entry guard.** PK `(pocomos_id, season)`; `pocomos_id` is the **INTERNAL** customer id (7-digit url_id — what `getDataset()` keys on, and directly usable for the note write; the external 6-digit number isn't available in bulk, §5.5b probe finding). The webhook inserts this row **FIRST**, before the PB move — if the move fails the guard still holds (the feeder reconciles the folder), and a re-fired webhook hits the PK and no-ops (`ON CONFLICT DO NOTHING`).

**Webhook fall-out (`webhookProcessor.ts` + webhook route).** A call is a wellness call when the contact's `category.category_id` == the Queue folder OR its custom fields carry `Hub Source = wellness` (`isWellnessContact()` — the field path covers hand-moved contacts). Flow, all inside the existing `waitUntil` (200-within-3s still holds): (1) insert `wellness_calls`; (2) `PUT /contacts/{id}` → Called folder (form-urlencoded; §4) + re-point `phoneburner_contacts.folder_id`; (3) Pocomos note **directly** to `/jwt/pronexis/{office}/customer/{id}/note/create` — the resolve step is SKIPPED (the stored Customer ID IS the url_id), subject `Wellness Call` (passed inside the note object; Pocomos may ignore it — unverified), summary = the standard §5.4 loop-guarded format. The note is written even when the parser's loop guard blanked the body — the call still happened. Every disposition counts, by design.

**Feeder — `src/lib/sync/wellnessFeed.ts` → `GET /api/cron/wellness-feed` (07:00, after the 06:00 refresh so counts are fresh).** Eligibility (ALL): (1) Active per the dashboard definition (status `Active` + ≥1 `"{CURRENT_YEAR} -"` tag, from `getDataset()` — same roster as the sweep); (2) `sprays_this_season >= 2`; (3) no `wellness_calls` row for the season; (4) phone not already in the Queue folder (normalized 10-digit, live listing — same as leadSync); (5) not `paused_balance` while `EXCLUDE_PAUSED_BALANCE = true` (one-line flip); (6) not a staff/internal record — `EXCLUDED_POCOMOS_IDS` (ops 2026-07-21: Brittany McAuliffe `1164546`). Push is form-urlencoded with `category_id = Queue`, THREE custom fields (`Customer ID` = internal id, `Pocomos Profile` = service-information URL, `Hub Source` = `wellness`), CSR-opener notes (sprays this season · last spray · sign-up date), `address1` = street from `respray_jobs.address` (rev 51 — the report's Address cell is **street-only**, no city/state/zip; zip comes from the dataset, city/state deliberately blank rather than guessed) — then a `phoneburner_contacts` upsert (`pocomos_type='customer'`, Queue folder) so notes-refresh Phase B (§5.1, now Queue-inclusive) keeps queue notes current. **Reconciliation belt-and-suspenders:** a Queue contact that already HAS a `wellness_calls` row (webhook recorded, move failed) is moved to Called instead of skipped silently. `?dryRun=1` = counts + would-push list, zero writes.

**Rollout state — LIVE (2026-07-21).** All five rollout steps done. Folders wired + sweep dry-run confirmed the wellness folders are never walked (9 policed folders only); counter filled + spot-checked (5/5); guard table + webhook deployed; feeder dry run reviewed by ops (Rivka) = roster 1,165 · 2+ sprays 1,137 · push list **1,113** (excluded: 9 paused-balance, 1 staff, 2 no-phone, 12 shared-phone duplicates; 1,096 bi-weekly / 17 weekly; Rivka Leyton deliberately KEPT). On the ops go the initial fill ran via `scripts/run-wellness-feed.ts --live` and the cron path dropped its `?dryRun=1` gate — the 07:00 feeder now pushes new qualifiers for real. First dial = a manual CSR test (nothing on the PB side is triggered beyond adding contacts). See the 2026-07-21 report for the fill numbers.

### 5.19b `/tv/board` v1 — original Pocomos-only board (rev 45, 2026-07-20)

Yodeck-ready shop-TV board replacing the paper/Google-Sheet route schedule. **URL:
`https://ms-operations-hub.vercel.app/tv/board`** — 1920×1080, SVG only (no emoji — Yodeck's Linux
browser), inline styles, self-refresh, **display-only (no writes)**.

**Layout:** today + the next 4 **workdays** (Sun–Fri; Saturday skipped) as 5 columns. Per day: a
weather cell (Open-Meteo, shared with `/tv/techs`), total stops, and a list of the day's route codes
(daycodes) each with area/towns and stop count.

**Data — what Pocomos provides (v1).** The schedule comes from `mosquito_service_status`:
`next_service_date` + `route_code` (== the sheet's daycode), grouped per day per route =
which routes run and how many stops. `routeTowns.ts` maps each daycode → area + towns (a **snapshot
of the sheet's DAYCODES tab**, probed 2026-07-20 — route geography is stable; superseded by the live
sheet when connected). The **Electric-Blower marker** reads the live `NT - Electric Blower Only` tag
(fast `customers`-table query, NOT `getDataset` — that rebuilds the ~80s roster and would hang a TV).

**⚠️ SHEET-DRIVEN PARTS ARE DORMANT.** `masterRouting.ts` reads the "2026 Master Routing List"
CALENDAR to overlay **tech names** and **ANT-day** assignments, but — like the referral scanner — it
needs the Drive/Sheets APIs enabled for the service account (see BACKLOG). Until then the board runs
fully off Pocomos + the snapshot, shows a `sheet not connected` badge, techs blank, and the ANT
"needs 3 dry days" rain caution (service-day + 2, ≥55% precip) stays dormant. All external reads
(weather, sheet) are 6s-timeout-guarded so nothing can stall the board. **v1 limitation:** a day with
more routes than fit shows the highest-volume ones (columns `overflow-hidden`).
