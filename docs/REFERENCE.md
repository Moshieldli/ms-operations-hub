...
- **Rule 2 — "RETURNED in year Y+1"** (the NUMERATOR, a COMBINED definition): a **real customer of Y+1 per rule 1**, **OR** holding a **Y+1 continuation tag (Auto / SEB / EB / Renewed) with ACTIVE status**. The tag path catches customers whose service rolled into the new season but who haven't been sprayed yet — without it an in-progress season understates returns. **ACTIVE status is required** so a cancelled customer's stale auto-renew tag can't count as a return (the rev-14 bug). The **denominator uses rule 1 ONLY** (no tag path).
...
**⚠ HISTORY: Pocomos data starts 2025 — the company ran RealGreen before that.** Every service-history-derived metric here is bounded by that floor as well as by the ~1-season render window below. **2024 service history is not in Pocomos at all**; it will come from a **RealGreen data dump** (the user will supply it). Until then 24→25 is not computable from any source we hold — see BACKLOG ("Import RealGreen dump → unblock 24→25 return rate"). Do not "fix" 24→25 by loosening the rule; the data is simply absent.

**Live numbers (verified via `getSalesTaxonomy()` + `scripts/verify-return-unification.ts` against the 100%-covered cache, 2026-07-16, rev 17):** **25→26 = 75.9% (976 / 1,286)** — of 1,286 real 2025 customers (rule 1), 976 returned (rule 2). **Numerator paths: 954 by continuation tag · 22 by spray history.** **Late-season signups counted as real: 2025 = 88, 2026 = 0** (today is before Aug 15, so no 2026 spray can be late yet — every real 2026 customer so far has 2+ sprays). Coverage 1,819 / 1,819 (100%; ~19 `table_ok=false`). **Returning box = 976** — Auto 404 · SEB 292 · EB 139 · Renewed 119 · by spray history 22 (sums to 976 ✓); 19 members are non-active (qualified purely on 2026 sprays); prior-year-real universe 1,286.
...
8. **`/sales` return rate — definition RESOLVED (rev 17: ≥2 sprays OR one after Aug 15; returned = real OR active continuation tag; Returning box unified with the numerator — §5.8); 24→25 BLOCKED on 2024 history, which is PRE-POCOMOS (RealGreen — dump pending from the user; see BACKLOG).** Built as a nightly resumable service-count scrape (`serviceCounts.ts` → `mosquito_service_counts`, 100% covered). **25→26 = 75.9% (976/1,286), live and exact.** **OPEN:** two independent floors block 24→25 — (a) **Pocomos data only starts 2025** (the company ran RealGreen before), and (b) the service-history table is **truncated to the most recent ~30 services (~1 season)**, so 2024 is gone for continuing customers regardless. **24→25 is not computable** (shows *n/a — needs full service history* on the card) until the RealGreen dump lands. The old idea of a full-history Pocomos source is moot for 2024 (nothing to read), though it would still help older pairs: parse the **PDF export** (`/customer/{id}/contract/{cid}/history/download` returns application/pdf), or find a paginated/date-ranged services endpoint. Probe: `scripts/probe-history-window.ts`.
