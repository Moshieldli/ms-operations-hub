...
**Reconciliation vs the "Returning" bucket + audit (rev 13).** `scripts/audit-return-gap.ts` (writes `audit-return-gap.csv`, gitignored) reconciles the 25→26 numerator against the `/sales` "Returning" (RETAINED) bucket. Findings 2026-07-07: numerator **1,079**, Returning bucket **1,004**, overlap **949**; every numerator member has a real 2025 mosquito contract + 2025 tag (**0 violations**); tag composition = 1,077 continuation-tag + 2 New-Sale re-signups. The **130-customer gap** (numerator NOT in Returning) is **not** re-signups as first assumed — it's **123 inactive + 5 on-hold + 2 active**: customers whose mosquito contract carries a 2026 continuation tag (e.g. `2026 - Auto`) but who have **since gone inactive/on-hold**. So the return numerator currently counts ~128 tagged-but-later-cancelled customers as "returned," which is exactly the mid-season/NT-cancel ambiguity in §9 open #8 — flagged with a card hint and a Needs-a-decision backlog item; not silently changed.
