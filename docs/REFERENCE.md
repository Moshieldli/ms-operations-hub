...
**RESPRAY WINDOW + MATURITY + TWO CLOCKS (rev 38, ops-confirmed).** Three linked rule changes;
together they move every rate on the board and this page.

**(1) The 9-day respray window — supersedes rev 24's windowless rule.** Ops: a re-service only ever
follows a spray within ~9 days; past that the crew pulls the customer's next scheduled Regular
earlier instead. So `RESPRAY_WINDOW_DAYS = 9`, and a re-service whose nearest prior spray is further
away is an **anomaly**: counted in the re-service total (it happened), blamed on **nobody**, and
listed on the page for ops to fix.

**Live effect: attributed resprays 99 → 60, team rate 1.84% → 1.11%, 39 anomalies.**

⚠️ **The data does NOT show a clean cliff at 9 days.** Gap distribution of the 99 re-services with a
prior job: 8d ×15, **9d ×14**, 10d ×11, 11d ×7, 12d ×5, 13d ×5, 14d ×1, 15d ×4, 16d ×3, 17d ×2,
20d ×1. The mode is 8–9 days, but the tail is real and continuous. Cumulative coverage by window:
**≤9d 61% · ≤10d 72% · ≤11d 79% · ≤12d 84% · ≤14d 90% · ≤17d 99%**. Calling 39% of re-services "data
anomalies" is a strong claim; if ops would rather capture the tail, `RESPRAY_WINDOW_DAYS` is a
one-constant change. Documented rather than silently widened.

**(2) Maturity.** A spray is **proven clean** once `MATURITY_DAYS` (9) have passed with no
re-service — the same number by construction, since once the window closes no respray can still
arrive. Clean streaks count **proven sprays only**; an immature spray has no verdict yet, and
counting it would inflate the streak of whoever sprayed most recently and then quietly revise it.

**(3) TWO CLOCKS.** These are genuinely different weeks, so **every award tile prints its own date
range** — that is what makes a mixed board self-explaining rather than wrong-looking:

| clock | drives | rule | on 2026-07-19 |
|---|---|---|---|
| **VOLUME** | Workhorse, Road Warrior, weekly spray counts, board table `sprays` | last completed Sun–Fri week (final at Friday close) | **Jul 12–17** |
| **MATURED** | Iron Wall, Perfect Week, Most Improved, respray rates, Clean Streak, board table `rate` | most recent Sun–Fri week where every spray is 9+ days old — exact, never revised | **Jul 5–10** |

`maturedWeekStart()` = the week containing `today − 14` (a week's Friday is `start + 5`; it is proven
when `today − (start+5) >= 9`). The matured clock necessarily runs ~1 week behind the volume clock.

**EXCLUSION IS AWARDS-ONLY (rev 38).** Cesar Barrerra (head tech, sporadic schedule) and the `Z-*`
route placeholders never win an award, but **their jobs count in every number**: YTD ticker, team
rate, weekly totals, cadence, season pace, and the `/service/resprays` table (Cesar keeps his row —
500 apps, 6 resprays, 1.20%). **The ticker now reads 5,393 YTD sprays, not 4,892** — the old figure
was silently dropping Cesar's 500 and the placeholders. Probed: `Z-ASAP 01` = 1 Regular sitting
inside a normal cadence sequence and `Z-RLW 01` = 1 genuine re-service 2 days after a Regular — both
**real work, not routing artifacts**, so both count.

**SEASON PACE (rev 38).** A card comparing YTD mosquito applications against the **same calendar
date** last season (from `completed_jobs_2025`, same Initial+Regular definition on both sides):
**5,393 vs 5,185 by 2025-07-19 — +4.0%.**
