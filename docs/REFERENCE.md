...
**Ops-canonical definition (2026-07-08, rev 15 — COMPLETED-SERVICE-COUNT based; supersedes the rev-14 tag/last-service model):** *"of customers who received at least `MIN_RETURN_TREATMENTS` (= 2) completed mosquito-family services (Event Spray EXCLUDED) in year Y, how many received at least 2 completed mosquito services in year Y+1?"* A return is a real treatment history — not a tag, not a one-off spray. The **same** test defines the denominator (real year-Y customer) and the numerator (returned in Y+1). `MIN_RETURN_TREATMENTS` + `RETURN_RATE_MIN_COVERAGE_PCT` are documented constants in `sales-taxonomy.ts`.
...
**Live numbers (verified via `getSalesTaxonomy()` against the 100%-covered cache, 2026-07-08, rev 15):** **25→26 = 75.9% (911 / 1,200)** — of 1,200 customers with ≥2 completed mosquito services in 2025, 911 already have ≥2 completed mosquito services in 2026 (season in progress, so this climbs). Coverage 1,816 / 1,816 (100%). **24→25 = n/a** (from-year outside the ~1-season service-history window). Compared to the rev-14 estimates (which were tag/last-service based): rev-14 had 25→26 = 72.6%; the rev-15 count-based figure is 75.9% because the "≥2 completed services" test is stricter on both sides and drops one-off/never-serviced tag holders from the denominator.
...
**`mosquito_service_counts`** (`pocomos_id, year, service_count`, PK `(pocomos_id, year)`) + **`mosquito_service_scrape`** (`pocomos_id` PK, `table_ok`, `scraped_at`) — added 2026-07-08. Per-year COMPLETED mosquito-family service counts (Event Spray excluded) feeding the return-rate metric (§5.8), plus the coverage tracker. Filled by the resumable `serviceCounts.ts` scrape (cron `/api/cron/service-counts`, `0 4 * * *`). 1,816 cohort members, 100% covered; ~19 `table_ok=false` (add-on default table not mosquito).
