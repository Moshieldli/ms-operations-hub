...
| `/api/cron/wellness-feed?dryRun=1` | `0 7 * * *` (07:00 daily) | Wellness-queue feeder — §5.21. **Dry-run-gated until the ops go** (drop the param to go live) |
...
**Rollout state (2026-07-21).** Steps 1–4 of the spec's rollout are done: folders wired + sweep dry-run confirmed the wellness folders are never walked (9 policed folders only, 0 would-moves); counter filled + spot-checked; guard table + webhook deployed (safe with an empty queue); feeder dry run = roster 1,165 · 2+ sprays 1,137 · **would-push 1,113** (excluded: 9 paused-balance, 1 staff, 2 no-phone, 12 shared-phone duplicates collapsed to one queue entry each; 1,096 of the 1,113 are bi-weekly). **The queue is NOT live-filled** — the shipped cron path is `?dryRun=1` (daily logged counts, no writes) until ops gives the go; going live = `scripts/run-wellness-feed.ts --live` + drop the cron param. See BACKLOG "Wellness live fill — GO/NO-GO".
