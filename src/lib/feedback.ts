...
/** List-row shape: everything but the heavy image, plus a `hasImage` flag. */
export type FeedbackListItem = Omit<FeedbackItem, "imageDataUri"> & { hasImage: boolean };
...
export async function createFeedback(input: NewFeedback): Promise<FeedbackItem> {
  ...
  const rows = (await sql`
    INSERT INTO feedback (body, submitter, source_url, image_data_uri)
    VALUES (${body}, ${submitter}, ${sourceUrl}, ${image})
    RETURNING id, body, submitter, source_url, image_data_uri, status,
  ...
  const rows = (status
    ? await sql`
        SELECT id, body, submitter, source_url,
               (image_data_uri IS NOT NULL) AS has_image, status,
               created_at::text, updated_at::text
    ...
    : await sql`
        SELECT id, body, submitter, source_url,
               ...
               created_at::text, updated_at::text
        FROM feedback
        ORDER BY created_at DESC`) as Array<Row & { has_image: boolean }>;
  return rows.map((r) => ({
    id: Number(r.id),
    body: r.body,
    submitter: r.submitter,
